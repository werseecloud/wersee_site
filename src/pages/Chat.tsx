...
const ChatSidebar = ({ activeRoom, setActiveRoom, filter, setFilter, rooms, loading, onOpenConnect }: any) => {
  ...
  const filteredRooms = rooms.filter((room: ChatRoom) => {
    ...
    return matchesFilter && (!normalizedSearch || room.name.toLowerCase().includes(normalizedSearch));
  });
...
const ChatWindow = ({ room, user }: { room: ChatRoom, user: any }) => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      if (error) {
        setSendError('Messages could not be loaded. Please try again.');
        return;
      }
    ...
    <div className="flex-1 flex flex-col h-full bg-[#F5F5F7]">
      ...
      {/* Input */}
        <div className="p-4 bg-white border-t border-black/5">
          {sendError && (
            <div className="mb-3 flex items-center gap-2 rounded-xl bg-red-50 px-3 py-2 text-sm text-red-700" role="alert">
...
export const Chat = () => {
  ...
  useEffect(() => {
    const fetchRoleAndChats = async () => {
      ...
      const [chatsResult, participantsResult] = await Promise.all([
        ...
        supabase
          ...
          .select(`
            chat_id,
            user_id,
            profile:profiles!chat_participants_user_id_profiles_fkey(
              id,
              full_name,
              username,
              avatar_url
      ...
      if (chatsResult.error || participantsResult.error) {
        console.error('Error loading conversations:', chatsResult.error || participantsResult.error);
        setLoadError('Your conversations could not be loaded.');
        setRooms([]);
        setLoading(false);
        return;
      ...
      const unreadByChat = new Map(
        (memberships || []).map((membership) => [membership.chat_id, membership.unread_count || 0])
      );
      const formattedRooms: ChatRoom[] = (chatsResult.data || []).map((chat: any) => {
        const participant = (participantsResult.data || []).find(
          (entry: any) => entry.chat_id === chat.id && entry.user_id !== user.id
        ) as any;
        const otherProfile = Array.isArray(participant?.profile)
          ? participant.profile[0]
          : participant?.profile;
        const isGroup = Boolean(chat.is_group || chat.team_id);

        return {
          id: chat.id,
          name: isGroup
            ? chat.name || 'Group conversation'
            : otherProfile?.full_name || otherProfile?.username || 'Wersee user',
          lastMessage: chat.last_message || 'No messages yet',
          time: new Date(chat.last_message_at || chat.updated_at || chat.created_at).toLocaleDateString(),
          unread: unreadByChat.get(chat.id) || 0,
          type: isGroup ? 'community' : 'product',
          status: chat.metadata?.status || 'active',
          avatar: otherProfile?.avatar_url || '',
          otherUserId: otherProfile?.id || participant?.user_id,
        };
      });
      ...
      setActiveRoom((currentRoom) => {
        const nextRoom = formattedRooms.find((room) => room.id === requestedChatId)
          || formattedRooms.find((room) => room.id === currentRoom?.id)
          || null;
        return nextRoom;
      });
      setLoading(false);
  ...
  const handleSelectRoom = (room: ChatRoom) => {
    setActiveRoom(room);
    setSearchParams({ chat: room.id }, { replace: true });
  };
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex bg-white">
      ...
      <div className="flex-1 flex lg:ml-20">
        ...
        />
        {loadError ? (
          <div className="flex flex-1 flex-col items-center justify-center bg-[#F5F5F7] p-8 text-center">
            <AlertCircle className="mb-4 h-10 w-10 text-red-500" />
            <h2 className="text-xl font-bold text-[#1D1D1F]">Messages unavailable</h2>
            <p className="mt-2 max-w-md text-gray-500">{loadError}</p>
          </div>
        ) : activeRoom ? (
          <>
