...
import { Settings, MapPin, Calendar, Edit3, Globe, Share2, Grid, List, User, Star, MessageCircle, Package, AlertCircle, CreditCard, LogOut, AlertTriangle, UserCheck, Plus, X, Loader2 } from 'lucide-react';
...
export const Profile = () => {
  ...
  const effectiveUsername = username
    ...
    : slugOrUsername?.startsWith('@')
      ? (() => {
          const accountHandle = parseAccountHandle(slugOrUsername);
          return accountHandle ? usernameFromAccountHandle(accountHandle) : null;
        })()
      : parseUsername(slugOrUsername);
    ...
    const fetchProfileData = async () => {
      ...
      try {
        ...
        if (effectiveUsername) {
          query = query.eq('username', effectiveUsername);
        } else if (username || slugOrUsername) {
          setProfile(null);
          return;
        } else if (id) {
        ...
        if (profileData) {
          ...
          const { data: reviewerProfiles, error: reviewersError } = reviewerIds.length
            ? await supabase
                .from('profiles')
          ...
          const reviewersById = new Map(
            (reviewerProfiles || []).map((reviewerProfile) => [reviewerProfile.id, reviewerProfile])
          );
          const reviewsData = (reviewRows || []).map((review) => ({
            ...review,
            reviewer: review.reviewer_id ? reviewersById.get(review.reviewer_id) || null : null,
          }));
  ...
  const profileSharePath = profile?.username
    ? routes.userProfile({ username: identifiers.username(profile.username) })
    : profile?.id
      ? routes.profileByUserId({ userId: identifiers.userId(profile.id) })
      : '/profile';
  const profileShareUrl = `https://www.wersee.com${profileSharePath}`;

  const handleShare = async () => {
    if (!profile) return;

    const shareData = {
      title: profile.seo_title || `${profile.full_name || profile.username} on Wersee`,
      text: profile.seo_description || profile.bio || `View @${profile.username || 'this profile'} on Wersee.`,
      url: profileShareUrl,
    };
    ...
    try {
      ...
      try {
        ...
        appToast('Could not share this profile.');
      }
    }
  };

  const handleStartChat = async () => {
    if (!profile || isOwnProfile || isStartingChat) return;
    if (!user) {
      navigate(`/auth?redirect=${encodeURIComponent(profileSharePath)}`);
      return;
    }
    ...
    try {
      ...
      if (!chatId) throw new Error('No conversation was created.');

      navigate(`/chat?chat=${encodeURIComponent(String(chatId))}`);
    } catch (error) {
      console.error('Error starting direct chat:', error);
      appToast(error instanceof Error ? error.message : 'Could not start this conversation.');
    } finally {
    ...
    if (listing.type === 'community' || listing.category === 'Education' || listing.metadata?.digitalType === 'course') {
      ...
      return (
        <ProductCard
          ...
          thumbnailB={listing.thumbnail_b_url}
          winningThumbnail={listing.winning_thumbnail}
          badge={listing.card_badge}
        />
      );
    }
  };
  ...
  return (
    <PageWrapper>
      <SEO 
        title={profile.seo_title || `${profile.full_name || profile.username} (@${profile.username || profile.id})`}
        description={profile.seo_description || profile.bio || `Discover ${profile.full_name || profile.username}'s products, services and creator profile on Wersee.`}
        image={profile.seo_image_url || `/api/profile-og?${profile.username ? `username=${encodeURIComponent(profile.username)}` : `id=${encodeURIComponent(profile.id)}`}`}
        imageAlt={`${profile.full_name || profile.username}'s profile on Wersee`}
        url={profileSharePath}
        type="profile"
        noIndex={!profile.is_public || !profile.is_indexable}
        jsonLd={{
          '@context': 'https://schema.org',
          '@type': 'Person',
          name: profile.full_name || profile.username,
          alternateName: profile.username ? `@${profile.username}` : undefined,
          description: profile.seo_description || profile.bio || undefined,
                ...
                <div className="flex flex-wrap justify-center md:justify-end gap-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={handleShare}
                      className="p-4 bg-white/5 text-white rounded-2xl hover:bg-white/10 transition-all backdrop-blur-xl border border-white/10 active:scale-95"
                      title="Share profile"
                      aria-label={`Share ${profile.full_name || profile.username}'s profile`}
                    >
                    ...
                    {!isOwnProfile && (
                      <>
                        <button
                          ...
                          className="p-4 bg-white/5 text-white rounded-2xl hover:bg-white/10 transition-all backdrop-blur-xl border border-white/10 active:scale-95 disabled:cursor-wait disabled:opacity-60"
