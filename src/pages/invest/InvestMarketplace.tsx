...
export default function InvestMarketplace() {
  ...
  const investBasePath = useMemo(() => {
    if (username && sessionId) return `/@${username}/invest/${sessionId}`;
    return '/invest';
  }, [username, sessionId]);
  ...
  useEffect(() => {
    ...
    const routeToUserInvest = async () => {
      const handle = await resolveUserHandle(user);
      if (cancelled) return;
      navigate(`/@${handle}/invest/${getInvestSessionId()}`, { replace: true });
    };
...
async function resolveUserHandle(user: any) {
  const fallback = slugifyHandle(user?.email?.split('@')[0] || user?.id || 'user');
  try {
    const { data } = await supabase.from('profiles').select('username,name').eq('id', user.id).maybeSingle();
    return slugifyHandle(data?.username || data?.name || fallback);
  } catch {
    return fallback;
  }
}
