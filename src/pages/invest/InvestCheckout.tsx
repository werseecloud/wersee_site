...
export default function InvestCheckout() {
  ...
  useEffect(() => {
    if (!user) navigate(`/auth?redirect=${encodeURIComponent(`/invest/${campaignSlug}/checkout`)}`, { replace: true });
  }, [user, navigate, campaignSlug]);
  ...
  const canContinue = Boolean(amountCents && campaign && RISK_ACCEPTANCE_TEXTS.every((_, index) => accepted[index]));
    ...
    <main className="min-h-screen bg-black pt-24 pb-16 text-white">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 sm:px-6 lg:grid-cols-[1fr_420px] lg:px-8">
        <section className="space-y-6">
          <Link to={`/invest/${campaign.slug}`} className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-white">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-6">
            ...
            {!clientSecret && (
              <button onClick={startPayment} disabled={!canContinue || loading} className="mt-6 w-full rounded-xl bg-emerald-500 px-5 py-4 text-sm font-black text-black disabled:cursor-not-allowed disabled:opacity-50">
                {loading ? 'Preparing payment...' : 'Go to secure payment'}
              </button>
            )}
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-6">
            ...
            <div className="mt-5 flex items-center gap-2 text-xs text-emerald-300"><CheckCircle2 className="h-4 w-4" /> Stripe Payment Element</div>
          </div>
        </aside>
      </div>
    </main>
