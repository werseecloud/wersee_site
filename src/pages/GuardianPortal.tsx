  ...
  const fetchChildren = async () => {
    try {
      ...
      const { data: relationships, error: relationshipsError } = await supabase
        .from('next_gen_relationships')
        .select('*, profiles:child_id(full_name, avatar_url, username, last_seen, kyc_status)')
      ...
      if (relationships && relationships.length > 0) {
        const kidIds = relationships.map(relationship => relationship.child_id);
        ...
        const enrichedChildren = relationships.map(relationship => {
          const kidListings = listings?.filter(l => l.seller_id === relationship.child_id) || [];
          const kidOrders = orders?.filter((o: any) => o.seller_id === relationship.child_id) || [];
          const kidBusiness = businesses?.find(b => b.user_id === relationship.child_id);
          const childName = relationship.profiles?.full_name || relationship.profiles?.username || 'Child';
          ...
          return {
            ...relationship,
            kid_id: relationship.child_id,
            kid_name: childName,
            stats: {
              revenue: completedRevenue,
              payouts: completedRevenue * 0.8, // 80% to creator/parent
              pending: pendingRevenue,
              lastLogin: relationship.profiles?.last_seen || null,
              kycStatus: relationship.profiles?.kyc_status || 'not_started',
              storeName: kidBusiness?.name || `${childName}'s Store`,
      ...
      if (requests) {
        const requestKidIds = [...new Set(requests.map(request => request.kid_id))];
        const { data: requestProfiles, error: requestProfilesError } = requestKidIds.length
          ? await supabase.from('profiles').select('id, full_name, username').in('id', requestKidIds)
          : { data: [], error: null };

        if (requestProfilesError) throw requestProfilesError;
        const requestProfileById = new Map(
          (requestProfiles || []).map(profile => [profile.id, profile])
        );

        setLogoutRequests(requests.map(req => ({
          id: req.id,
          kidName: requestProfileById.get(req.kid_id)?.full_name
            || requestProfileById.get(req.kid_id)?.username
            || 'Your child',
