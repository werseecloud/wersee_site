...
import { Loader2, ShoppingBag, CheckCircle2, AlertCircle, ArrowLeft, ShieldCheck, CreditCard } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { SEO } from '../components/SEO';
import { PointsWalletSlider, usePointsWallets } from '../components/payments/PointsWalletSlider';
import { WerseePointsMark } from '../components/payments/WerseePointsMark';
        ...
        <div className="space-y-6">
          <div className="p-6 bg-yellow-500/5 border border-yellow-500/10 rounded-2xl space-y-4">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-500 font-medium">Selected card balance</span>
              <span className="font-mono font-bold text-yellow-600">{Number(selectedWallet?.balance_points || 0).toLocaleString()} Points</span>
            </div>
            <div className="flex items-center justify-between">
            ...
            {userId && selectedWallet && Number(selectedWallet.balance_points) < (checkout.total_amount * 100) && (
          ...
          <button
            ...
            disabled={loading || !userId || !selectedWallet || Number(selectedWallet.balance_points) < (checkout.total_amount * 100)}
...
export const PosCheckoutPage = () => {
  const { accountHandle, systemname, checkout_name, checkout_id, id } = useParams();
