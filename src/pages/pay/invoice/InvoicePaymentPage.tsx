...
export default function InvoicePaymentPage() {
  ...
  const handlePay = async () => {
    ...
    try {
      const resData = await invokeApiRunner('invoice-checkout', {
  ...
  if (error || !invoice) {
    return (
      <div className="min-h-screen bg-[#F2F2F7] flex items-center justify-center p-4">
        <div className="bg-white rounded-[2rem] p-8 max-w-md w-full text-center shadow-sm">
          ...
          <button onClick={() => navigate('/')} className="w-full py-4 bg-black text-white rounded-2xl font-bold">
            Go Home
          </button>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-[#F2F2F7] flex items-center justify-center p-4">
        <div className="bg-white rounded-[2rem] p-8 max-w-md w-full text-center shadow-sm">
          ...
          <p className="text-gray-500 mb-6">This invoice has already been settled. Thank you!</p>
          <button onClick={() => navigate('/')} className="w-full py-4 bg-black text-white rounded-2xl font-bold">
            Done
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] lg:bg-white">
      <SEO 
        title={`Pay Invoice ${invoice.invoice_number} - ${metadata.business_name || 'Wersee'}`}
        description={`Securely pay invoice ${invoice.invoice_number} for ${currencySymbol}${invoice.amount.toFixed(2)} from ${metadata.business_name || 'Wersee Seller'}.`}
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tight text-black leading-none">
                  {metadata.business_name || 'Checkout'}
                </h1>
                <p className="text-gray-500 font-medium mt-1">Invoice {invoice.invoice_number}</p>
              </div>
            </div>
      ...
      <div className="lg:hidden min-h-screen bg-[#F5F5F7] flex flex-col font-sans">
        ...
        <div className="px-6 pt-12 pb-4 flex justify-between items-center bg-[#F5F5F7] sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium">Invoice {invoice.invoice_number}</p>
            </div>
          </div>
          <button 
            onClick={() => navigate('/')}
            className="w-8 h-8 bg-black/5 rounded-full flex items-center justify-center text-gray-600 hover:bg-black/10 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="px-6 py-8 flex flex-col items-center justify-center text-center">
          <span className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-2">Amount Due</span>
        ...
        <div className="mt-auto bg-white rounded-t-[2.5rem] shadow-[0_-8px_40px_rgba(0,0,0,0.08)] p-6 pb-safe flex flex-col">
          ...
          >
            {paying ? (
              ...
              <>
                Pay {currencySymbol}{payMode === 'later' ? totalWithFee.toLocaleString('en-US', { minimumFractionDigits: 2 }) : Number(invoice.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}
              </>
            )}
          </button>
