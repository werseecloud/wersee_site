  ...
  useEffect(() => {
    async function loadAuthDetails() {
      ...
      if (!user) {
        ...
        navigate(`/auth?redirect=${encodeURIComponent(`/oauth/consent?authorization_id=${authorizationId}`)}`);
        return;
      }

      try {
        // Get authorization details using the authorization_id
        // Note: We are using the any type here because the supabase-js types might not be fully updated 
        // for this beta feature in the current environment
              ...
              <ul className="space-y-3">
                {authDetails.scope.split(' ').map((scopeItem: string) => (
                  <li key={scopeItem} className="flex items-start gap-3 text-[#1D1D1F]">
                    ...
                    <span className="font-medium">{scopeItem}</span>
                  </li>
                ))}
              </ul>
            </div>
