...
import { Loader2, AlertCircle, CheckCircle2, ShieldCheck, ArrowLeft, FileText, CreditCard, Sparkles } from 'lucide-react';
import { motion } from 'motion/react';
import { SEO } from '../SEO';

const CheckoutForm = ({ invoice, clientSecret, returnUrl, isSandbox }: { invoice: any, clientSecret: string, returnUrl: string, isSandbox: boolean }) => {
  ...
  const handlePointsPayment = async () => {
    ...
    if (userProfile.wersee_points < pointsNeeded && !isSandbox) {
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-3">
        ...
        {payWithPoints ? (
          <div className="p-6 bg-white/5 border border-white/10 rounded-2xl space-y-4">
            ...
            {!userProfile && !isSandbox && (
              <p className="text-xs text-red-400">Please log in to use your points.</p>
            )}
            {userProfile && userProfile.wersee_points < (invoice.amount * 100) && !isSandbox && (
      ...
      <button
        ...
        disabled={(!payWithPoints && !stripe && !isSandbox) || processing || (payWithPoints && !isSandbox && (!userProfile || userProfile.wersee_points < (invoice.amount * 100)))}
  ...
  useEffect(() => {
    const fetchInvoiceAndSetupPayment = async () => {
      try {
        ...
        const returnUrl = invoiceData.invoice_number 
          ? `${window.location.origin}/quick-pay/invoice/${invoiceData.invoice_number}`
          : `${window.location.origin}/${invoiceData.username}/invoice/${invoiceData.slug}`;
  ...
  if (invoice?.status === 'paid') {
      return (
        <div className="min-h-screen bg-[#0A0A0A] flex flex-col items-center justify-center p-6 text-center">
            ...
            <button 
                onClick={() => navigate(`/${username}/invoice/${slug}`)}
                className="mt-6 px-6 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors"
            >
                View Invoice
            </button>
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col md:flex-row">
      <SEO 
        ...
        url={`/@${username}/quick-pay/invoice/${slug}`}
        noIndex={true}
      />
        ...
        <div className="max-w-md mx-auto w-full">
          ...
          {isSandbox ? (
            <CheckoutForm 
              invoice={invoice} 
              clientSecret="" 
              returnUrl={invoice.returnUrl || `${window.location.origin}/${username}/invoice/${slug}`} 
              isSandbox={isSandbox}
            />
