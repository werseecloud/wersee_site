...
const SubscriptionForm = ({ subscription, returnUrl, retryUrl }: { subscription: any, returnUrl: string, retryUrl: string }) => {
    ...
    try {
      ...
      if (submitError) {
        ...
        window.location.assign(
          `/payment-success?source=subscription&status=${status}&return_to=${encodeURIComponent(retryUrl)}`,
        );
      }
    } catch {
      window.location.assign(
        `/payment-success?source=subscription&status=error&return_to=${encodeURIComponent(retryUrl)}`,
      );
    }
  };
...
export const SubscriptionPublicView = () => {
  ...
  const subscriptionPath = routeUsername && slug
    ? routes.userSubscription({ username: routeUsername, subscriptionSlug: slug })
    : '/';
  ...
  useEffect(() => {
    const fetchSubscription = async () => {
      try {
        ...
        if (!pk) throw new Error('Secure checkout is not configured.');
  ...
  const handleStartSubscription = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setPaymentSetupError(err.message || 'Secure checkout could not start.');
    } finally {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#050505] text-white relative overflow-hidden font-sans">
      <SEO 
      ...
      <motion.div 
        ...
        className="relative z-10 grid min-h-[100dvh] w-full bg-[#0A0A0A]/80 backdrop-blur-3xl md:grid-cols-[minmax(320px,42%)_1fr]"
      >
        {/* Left Side: Plan Info */}
        <div className="relative flex w-full flex-col justify-between border-b border-white/10 bg-white/[0.02] p-7 sm:p-12 md:min-h-[100dvh] md:border-b-0 md:border-r lg:p-16">
          <div>
        ...
        <div className="flex w-full flex-col justify-center bg-[#0A0A0A] px-6 py-12 sm:p-12 lg:p-16">
              ...
              {step === 'details' ? (
                ...
                >
                  ...
                  >
                    ...
                    <p className="mb-8 mt-2 text-sm leading-relaxed text-gray-400">
                      Choose one of the secure methods available for your country and this subscription.
                    </p>
                  ...
                  {clientSecret && stripePromise ? (
                    <Elements stripe={stripePromise} options={{ clientSecret, appearance: appearance as any }}>
                      <SubscriptionForm 
                        ...
                        returnUrl={`${window.location.origin}/payment-success?source=subscription&subscription_id=${encodeURIComponent(subscription.id)}&return_to=${encodeURIComponent(subscriptionPath)}`}
                        retryUrl={subscriptionPath}
                      />
                    </Elements>
                  ) : paymentSetupError ? (
                    <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-5" role="alert">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="font-bold text-red-100">Payment methods unavailable</p>
                          <p className="mt-1 text-sm leading-relaxed text-red-100/65">{paymentSetupError}</p>
                        </div>
                      </div>
                      <button
                        type="button"
                        onClick={() => setStep('details')}
                        className="mt-5 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-black"
                      >
                        Try again
                      </button>
                    </div>
                  ) : (
                    <div className="flex min-h-40 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.02]">
