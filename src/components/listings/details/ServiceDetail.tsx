                  ...
                  <div className="flex gap-4 min-w-max">
                    {packages.map((pkg: any, idx: number) => (
                      ...
                      >
                        ...
                        <LocalizedPrice amount={pkg.price} baseCurrency={pkg.currency || listing.currency || listing.base_currency || 'EUR'} className="text-2xl font-bold mb-4 text-white" showOriginal={false} />
              ...
              <div className="bg-[#141414] p-8 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-8">
                <div className="space-y-4">
                  ...
                  <div className="flex items-baseline gap-1">
                    {isEditing ? (
                      ...
                      <LocalizedPrice amount={packages[selectedPackage]?.price || listing.price} baseCurrency={packages[selectedPackage]?.currency || listing.currency || listing.base_currency || 'EUR'} className="text-4xl font-bold text-white" />
                    )}
                  </div>
                ...
                {!isEditing && (
                  <div className="space-y-4 pt-4">
                    ...
                    >
                      {canBuy ? (
                        listing.pricing_type === 'subscription' ? 'Subscribe' : (
                          <>Continue (<LocalizedPrice amount={packages[selectedPackage]?.price || listing.price} baseCurrency={packages[selectedPackage]?.currency || listing.currency || listing.base_currency || 'EUR'} showOriginal={false} />)</>
                        )
                      ) : 'Payments Disabled'}
                    </button>
                    <button 
