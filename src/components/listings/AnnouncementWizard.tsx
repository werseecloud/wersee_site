...
export const AnnouncementWizard = ({ onClose, onCreated, draftId }: AnnouncementWizardProps) => {
    ...
    if (step === 1 && !draftId && user) {
      try {
        ...
        if (data) {
          ...
          replaceStandaloneListingDraftUrl('announcement', data.id);
        }
      } catch (error) {
  ...
  return (
    <WizardLayout
      ...
      aiKind="announcement"
      aiCurrentDraft={{ title: formData.title, description: formData.description, category: formData.category, price: 0 }}
      onAiApply={(patch) => setFormData((current) => ({ ...current, ...(typeof patch.title === 'string' ? { title: patch.title } : {}), ...(typeof patch.description === 'string' ? { description: patch.description } : {}), ...(typeof patch.category === 'string' ? { category: patch.category } : {}) }))}
    >
      {step === 1 && (
