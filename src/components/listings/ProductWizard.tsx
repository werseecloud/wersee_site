...
import { getCurrentSellerDsaVerification, isDsaVerificationComplete, requiresDsaSellerVerification } from '../../lib/dsaCompliance';
...
export const ProductWizard = ({ onClose, isAffiliate = false, onCreated, draftId }: ProductWizardProps) => {
  const { user } = useAuth();
  // The fullscreen editor intentionally uses one neutral dark canvas in every
  // site theme, so form controls must not inherit light-theme contrast tokens.
  const isDark = true;
  ...
  const stepCopy = isAffiliate ? [
    { title: 'Describe the product', description: 'Give buyers a clear reason to click through.' },
    { title: 'Add the offer', description: 'Connect the destination, price and product imagery.' },
    { title: 'Make it convincing', description: 'Add buyer context and answers before publishing.' },
  ] : [
    { title: 'Start with the essentials', description: 'Name the product and explain what makes it worth buying.' },
    { title: 'Set pricing and inventory', description: 'Keep the base offer simple, then add variations only when needed.' },
    { title: 'Plan delivery', description: 'Tell buyers where it ships from, when it arrives and what it costs.' },
    { title: 'Handle tax and customs', description: 'Add the details needed for accurate cross-border selling.' },
    { title: 'Set buyer protection', description: 'Make returns and warranty expectations clear before checkout.' },
    { title: 'Choose a thumbnail', description: 'Use one strong square image for product cards and search.' },
    { title: 'Finish the product page', description: 'Add supporting images and an optional pickup address.' },
    { title: 'Strengthen the offer', description: 'Answer likely questions and define who this product is for.' },
  ];
  ...
  useEffect(() => {
    ...
    getCurrentSellerDsaVerification(user.id)
      .then(setDsaVerification)
      .catch((error) => console.warn('Could not load DSA seller verification:', error));
  }, [user?.id]);
    ...
    if (step === 1 && !draftId && user) {
      try {
        ...
        if (data) {
          ...
          replaceStandaloneListingDraftUrl(isAffiliate ? 'affiliate' : 'product', data.id);
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      if (formData.salePrice && Number(formData.salePrice) >= Number(formData.variations[0]?.price || 0)) {
      ...
      const listingData = {
        ...
        description: formData.description,
        price: parseFloat(formData.variations[0]?.price) || 0,
        original_price: parseFloat(formData.variations[0]?.price) || 0,
        ...
        status: 'published',
        images: finalImages,
        dsa_seller_verification_status: dsaVerification?.status || 'unknown',
        metadata: {
          ...formData,
          shipping_origin: formData.originCountry,
          external_url: isAffiliate ? formData.externalUrl : undefined,
          dsa_seller_country: dsaVerification?.country_code || formData.originCountry,
          dsa_trader_status: dsaVerification?.trader_status || 'consumer',
        }
      };

      if (requiresDsaSellerVerification({
        countryCode: dsaVerification?.country_code || formData.originCountry,
        traderStatus: dsaVerification?.trader_status,
      }) && !isDsaVerificationComplete(dsaVerification)) {
        toast.error("Complete EU seller verification before publishing this product.");
        return;
      }
  ...
  return (
    <WizardLayout
      ...
      stepTitle={stepCopy[step - 1]?.title}
      stepDescription={stepCopy[step - 1]?.description}
      currentStep={step}
      totalSteps={isAffiliate ? 3 : 8}
      onClose={onClose}
      onBack={handleBack}
      onNext={handleNext}
      isFirstStep={step === 1}
      isLastStep={step === (isAffiliate ? 3 : 8)}
      loading={loading}
      variant="fullscreen"
      preview={(type, isDarkPreview) => <ProductPreview data={formData} isDark={isDarkPreview} isAffiliate={isAffiliate} type={type} />}
      aiKind="physical"
      aiCurrentDraft={{ title: formData.title, description: formData.description, category: formData.category, price: Number(formData.variations[0]?.price) || 0, faqs: formData.faq }}
      onAiApply={(patch) => setFormData((current) => ({ ...current, ...(typeof patch.title === 'string' ? { title: patch.title } : {}), ...(typeof patch.description === 'string' ? { description: patch.description } : {}), ...(typeof patch.category === 'string' ? { category: patch.category } : {}), ...(Array.isArray(patch.faqs) ? { faq: patch.faqs as { question: string; answer: string }[] } : {}), variations: current.variations.map((variation, index) => index === 0 && typeof patch.price === 'number' ? { ...variation, price: String(patch.price) } : variation) }))}
    >
      {step === 1 && (
        <div className="space-y-6">
          <div>
      ...
      {!isAffiliate && step === 2 && (
        <div className="space-y-6">
          <div className={`grid gap-4 rounded-2xl border p-4 sm:grid-cols-2 ${isDark ? 'border-white/10 bg-white/[0.03]' : 'border-gray-200 bg-gray-50'}`}>
            <div><label className={labelClass}>Original price (€)</label><input type="number" min="0" step="0.01" value={formData.variations[0].price} onChange={(event) => updateVariation(0, 'price', event.target.value)} placeholder="99.00" className={inputClass} /></div>
            <div><label className={labelClass}>Sale price (€)</label><input type="number" min="0" step="0.01" value={formData.salePrice} onChange={(event) => updateField('salePrice', event.target.value)} placeholder="Optional" className={inputClass} />{formData.salePrice && Number(formData.salePrice) < Number(formData.variations[0].price) && <p className="mt-2 text-xs font-bold text-emerald-500">Save €{(Number(formData.variations[0].price) - Number(formData.salePrice)).toFixed(2)} · {Math.round(((Number(formData.variations[0].price) - Number(formData.salePrice)) / Number(formData.variations[0].price)) * 100)}% OFF</p>}</div>
          </div>
