...
export const DigitalWizard = ({ onClose, isVirtual = false, onCreated, draftId }: DigitalWizardProps) => {
    ...
    if (step === 1 && !draftId && user) {
      try {
        ...
        if (data) {
          ...
          replaceStandaloneListingDraftUrl(isVirtual ? 'virtual' : 'digital', data.id);
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      if (formData.salePrice && Number(formData.salePrice) >= Number(formData.price || 0)) {
    ...
    <>
      {showWizard && (
        <WizardLayout
          ...
          aiKind={formData.digitalType === 'course' ? 'course' : 'digital'}
          aiCurrentDraft={{ title: formData.title, description: formData.description, category: formData.category, price: Number(formData.price) || 0, faqs: formData.faq }}
          onAiApply={(patch) => setFormData((current) => ({ ...current, ...(typeof patch.title === 'string' ? { title: patch.title } : {}), ...(typeof patch.description === 'string' ? { description: patch.description } : {}), ...(typeof patch.category === 'string' ? { category: patch.category } : {}), ...(typeof patch.price === 'number' ? { price: String(patch.price) } : {}), ...(Array.isArray(patch.faqs) ? { faq: patch.faqs as { question: string; answer: string }[] } : {}) }))}
        >
          {step === 1 && (
            <div className="space-y-6">
              ...
              <div>
                ...
                <input type="number" min="0" step="0.01" value={formData.salePrice} onChange={(e) => updateField('salePrice', e.target.value)} placeholder="Optional" className={inputClass} />
                {formData.salePrice && Number(formData.salePrice) < Number(formData.price) && <p className="mt-2 text-xs font-bold text-emerald-500">Save €{(Number(formData.price) - Number(formData.salePrice)).toFixed(2)} · {Math.round(((Number(formData.price) - Number(formData.salePrice)) / Number(formData.price)) * 100)}% OFF</p>}
              </div>
