...
import { parseUsernameRouteValue, tryUserProductPath } from '../routing/routes';
            ...
            <div className="flex-1 pr-4">
              <p className="text-sm text-gray-800 leading-snug">
                <span className="font-bold text-black">
                  {parseUsernameRouteValue(activity.username)
                    ? `@${parseUsernameRouteValue(activity.username)}`
                    : activity.username}
                </span>
              ...
              <button 
                onClick={() => {
                  if (activity.productSlug && activity.type === 'listing') {
                    const destination = tryUserProductPath(activity.username, activity.productSlug);
                    navigate(destination || `/search?q=${encodeURIComponent(activity.productName)}`);
                  } else if (activity.productSlug && activity.type === 'purchase' && activity.sellerUsername) {
                    const destination = tryUserProductPath(activity.sellerUsername, activity.productSlug);
                    navigate(destination || `/search?q=${encodeURIComponent(activity.productName)}`);
