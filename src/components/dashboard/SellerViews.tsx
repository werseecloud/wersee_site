...
export const SellerOrders = ({ user }: { user: any }) => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('*, listing:listings(title)')
        .eq('seller_id', user.id)
        .or('type.is.null,type.neq.job_application')
        .order('created_at', { ascending: false });
  ...
  const exportOrdersCsv = () => {
    ...
    const headers = [
      'Order ID', 'Public order code', 'Created at', 'Product', 'Customer email',
      'Amount', 'Currency', 'Order status', 'Payment status', 'Shipping status',
      'Country', 'Refund status',
    ];
    const rows = orders.map((order) => [
      order.id,
      order.public_order_code,
      order.created_at ? new Date(order.created_at).toISOString() : '',
      order.listing?.title || 'Deleted or unavailable listing',
      order.customer_email || order.buyer_email || '',
      Number(order.total_amount ?? order.amount ?? 0).toFixed(2),
      String(order.buyer_currency || order.currency || 'EUR').toUpperCase(),
      order.status || '',
      order.payment_status || '',
      order.shipping_status || '',
      order.buyer_country || order.country || '',
      order.refund_status || '',
    ]);
    ...
    anchor.href = url;
    anchor.download = `wersee-orders-${new Date().toISOString().slice(0, 10)}.csv`;
    document.body.appendChild(anchor);
    anchor.click();
    anchor.remove();
    window.setTimeout(() => URL.revokeObjectURL(url), 1000);
    appToast(`${orders.length} order${orders.length === 1 ? '' : 's'} exported.`, 'success');
  };
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-[#1D1D1F] p-6 sm:p-8 rounded-3xl border border-black/5 dark:border-white/5 shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <button onClick={exportOrdersCsv} disabled={loading || Boolean(loadError)} className="w-full sm:w-auto px-4 py-2 bg-black dark:bg-white text-white dark:text-black rounded-xl font-medium hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors disabled:cursor-not-allowed disabled:opacity-40">
            Export CSV
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-red-500/15 bg-red-500/5 p-6 text-center text-sm text-red-500 dark:text-red-300">
            {loadError}
          </div>
        ) : (
          <>
