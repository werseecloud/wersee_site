...
export const TikTokFeed = () => {
  ...
  const loadFeed = async () => {
    ...
    const listingIds = (data || []).map((item: any) => item.id).filter(Boolean);
    const { data: offers } = listingIds.length ? await supabase.from('product_offers').select('*').in('listing_id', listingIds) : { data: [] as any[] };
    setFeed((data || []).map((item: any) => ({ ...item, product_offers: (offers || []).filter((offer: any) => offer.listing_id === item.id) })));
  ...
  const pricing = resolveProductPricing(currentListing);
        ...
        >
          ...
          <div className="relative z-20 flex justify-between items-end w-full">
            <div className="flex-2 pr-4">
              ...
              <div className="flex items-center gap-2"><p className="text-2xl text-[#F0D080] font-light">{formatProductPrice(pricing.currentPrice)}</p>{pricing.isOnSale && <><span className="text-sm text-white/40 line-through">{formatProductPrice(pricing.originalPrice)}</span><span className="rounded-full bg-white px-2 py-1 text-xs font-black text-black">-{pricing.discountPercent}%</span></>}</div>
              ...
              <div className="flex items-center gap-2 mt-4 text-xs text-gray-400 bg-white/5 w-max px-3 py-2.5 rounded-full backdrop-blur-sm border border-white/20">
                ...
                <span>{pricing.offer ? `${pricing.offer.name} · ${formatOfferTimeRemaining(pricing.offer)}` : 'Personalized for your interests'}</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-5 items-center pb-2">
              ...
              <button 
                onClick={(e) => { e.stopPropagation(); void trackProductConversion(String(currentListing.id), 'click', 'feed'); navigate(`/listing/${currentListing.id}`); }}
