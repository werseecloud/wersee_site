...
export const ManagementDashboard = ({ onNavigate }: { onNavigate: (view: string) => void }) => {
  ...
  const [teamActivity, setTeamActivity] = useState<ManagementActivityItem[]>([]);
  ...
  const fetchManagementData = async () => {
    ...
    try {
      ...
      const databaseActivity: ManagementActivityItem[] = [
        ...(products || []).map((product) => ({
          id: `product-${product.id}`,
          user: 'You',
          action: 'published product',
          target: product.title || 'Untitled product',
          occurredAt: product.created_at,
        })),
        ...(tasks || []).map((task) => ({
          id: `task-${task.id}`,
          user: task.assigned_to === user?.id ? 'You' : 'Team',
          action: task.completed_at || task.status === 'done' ? 'completed task' : 'created task',
            ...
            <div className="space-y-4">
                ...
                <div key={act.id} className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-500 mt-0.5">
                      {formatDistanceToNow(new Date(act.occurredAt), { addSuffix: true })}
                    </p>
                  </div>
                </div>
              ))}
              {teamActivity.length === 0 && (
                <div className="rounded-2xl border border-dashed border-white/10 px-4 py-8 text-center">
                  <Activity className="mx-auto mb-3 h-5 w-5 text-gray-600" />
                  <p className="text-xs text-gray-500">Product and team-task activity will appear here.</p>
                </div>
              )}
            </div>
          </div>
