...
export const MoneyCouponsView = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [couponsData, promoData] = await Promise.all([
        invokeFinanceWorkflow<{ data: any[] }>('coupon-list'),
        invokeFinanceWorkflow<{ data: any[] }>('promotion-code-list'),
      ]);
  ...
  const handleCreateCoupon = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Create Coupon
      const couponResult = await invokeFinanceWorkflow<{ coupon: any }>('coupon-create', {
        name: name || undefined,
      ...
      if (promoCode && coupon?.id) {
        await invokeFinanceWorkflow('promotion-code-create', {
          coupon_id: coupon.id,
          code: promoCode,
          max_redemptions: maxRedemptions ? parseInt(maxRedemptions) : undefined,
        });
      ...
      setPromoCode('');
      await fetchData();
      appToast('Discount created');
    } catch (error) {
      console.error('Error creating coupon:', error);
      appToast(error instanceof Error ? error.message : 'Failed to create coupon.');
    } finally {
      setIsCreating(false);
    }
  };
