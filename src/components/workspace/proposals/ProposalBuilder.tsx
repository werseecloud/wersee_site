...
export const ProposalBuilder: React.FC<ProposalBuilderProps> = ({ proposalId, onClose }) => {
  ...
  const getUserBusinessId = async (userId: string) => {
    ...
    if (teamMember?.business_id) return teamMember.business_id;
  ...
  const handleSave = async (status: 'draft' | 'sent') => {
    try {
      ...
      if (status === 'sent') {
        const url = `${window.location.origin}/proposal/${currentProposalId}`;
        await navigator.clipboard.writeText(url);
        appToast('Proposal sent! Link copied to clipboard.');
        onClose();
      } else {
      ...
      appToast('Failed to save proposal.');
    } finally {
      setIsSaving(false);
    }
  };
