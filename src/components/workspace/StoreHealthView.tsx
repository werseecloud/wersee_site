  ...
  <div className="mx-auto max-w-7xl space-y-6 pb-16 text-white">
    <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
      <div className="space-y-3">
        ...
        <div className="h-12 w-72 animate-pulse rounded-lg bg-white/5 md:w-96" />
    ...
    <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {[0, 1, 2, 3].map((item) => (
          <div key={item} className="h-40 animate-pulse rounded-lg border border-white/10 bg-white/[0.03]" />
        ))}
      </div>
    </div>
  </div>
...
export const StoreHealthView = ({ businessId: initialBusinessId }: { businessId?: string }) => {
  ...
  const [pageSpeedAvailable, setPageSpeedAvailable] = useState<boolean | null>(null);
  const [availabilityMessage, setAvailabilityMessage] = useState('');

  const selectedBusiness = businesses.find((business) => business.id === selectedBusinessId) || null;
  const scansForStrategy = useMemo(() => scans.filter((scan) => scan.strategy === strategy), [scans, strategy]);
  const latestScan = scansForStrategy[0] || null;
  const lastScanDate = scans[0]?.scanned_at;

  const scoreTrend = useMemo(() => scansForStrategy.slice(0, 8).reverse(), [scansForStrategy]);

  const loadAvailability = async () => {
    const { data, error: statusError } = await supabase.functions.invoke('store-health-scan', {
      body: { action: 'status' },
    });

    if (statusError) {
      setPageSpeedAvailable(false);
      setAvailabilityMessage('Store Health kon de PageSpeed-configuratie niet controleren. Probeer het later opnieuw.');
      return;
    }

    const available = data?.available !== false;
    setPageSpeedAvailable(available);
    setAvailabilityMessage(available ? '' : data?.message || 'Google PageSpeed is nog niet geconfigureerd.');
  };
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const initialBusinessExists = initialBusinessId && nextBusinesses.some((business) => business.id === initialBusinessId);
      const firstBusinessId = (initialBusinessExists ? initialBusinessId : selectedBusinessId) || nextBusinesses[0]?.id || '';
  ...
  const runScan = async () => {
    ...
    if (pageSpeedAvailable === false) {
      setError(availabilityMessage || 'Google PageSpeed is nog niet geconfigureerd.');
      return;
    }
    setScanning(true);
    setError('');
    ...
    try {
      const { data, error: functionError } = await supabase.functions.invoke('store-health-scan', {
        body: { businessId: selectedBusinessId, strategy: 'both' },
      });
      ...
      if (!data?.success) {
        if (data?.code === 'PAGESPEED_NOT_CONFIGURED') {
          setPageSpeedAvailable(false);
          setAvailabilityMessage(data.error);
        }
        throw new Error(data?.error || 'PageSpeed scan is mislukt.');
      }

      await loadScans(selectedBusinessId);
      setNotice('Nieuwe mobiele en desktop Store Health scans opgeslagen.');
    } catch (err: any) {
  ...
  if (loading && businesses.length === 0) return <StoreHealthSkeleton />;
    ...
    <div className="mx-auto max-w-7xl space-y-6 pb-16 text-white">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-gray-400 md:text-base">
            Scan je publieke Wersee Store met Google PageSpeed Insights en volg performance, accessibility, SEO en best practices voor mobiel en desktop.
          </p>
          <p className="mt-2 text-xs font-bold uppercase tracking-wide text-gray-500">
            Laatste scan: {formatDate(lastScanDate)}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <button
            onClick={runScan}
            disabled={!selectedBusinessId || scanning || pageSpeedAvailable === false}
            title={pageSpeedAvailable === false ? availabilityMessage : undefined}
          ...
          >
            ...
            {scanning ? 'Scanning mobile + desktop...' : pageSpeedAvailable === false ? 'PageSpeed setup required' : 'Run new scan'}
          </button>
        </div>
      </div>
      ...
      {scanning && !latestScan ? (
        <div className="space-y-4">
          <div className="flex items-center gap-3 rounded-lg border border-blue-500/20 bg-blue-500/10 p-4 text-sm font-bold text-blue-200">
            <Loader2 className="h-5 w-5 animate-spin" />
            Google PageSpeed scant mobiel en desktop. Dit kan even duren.
          </div>
          <StoreHealthSkeleton />
        </div>
      ) : businesses.length === 0 ? (
              ...
              <div className="mb-6 flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm text-gray-400">
                    Laatste {strategy} scan: {formatDate(latestScan.scanned_at)}
                  </p>
                </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            {scanning ? 'Scanning mobile + desktop...' : 'Run new scan'}
          </button>
        </div>
      )}
    </div>
