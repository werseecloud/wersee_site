...
export const WebsitesBuilderView = () => {
  ...
  const fetchWebsites = async () => {
    setLoading(true);
    try {
      const { data, error } = await DatabaseService.get('websites', {
  ...
  const fetchSections = async (websiteId: string) => {
    try {
      const { data, error } = await DatabaseService.get('website_sections', {
  ...
  const handleCreateWebsite = async () => {
    ...
    setSaving(true);
    try {
      const { data, error } = await DatabaseService.insert('websites', { 
        name: newWebsiteName, 
        user_id: user?.id,
        status: 'draft'
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (sectionsToInsert.length > 0) {
        const { error: insertError } = await DatabaseService.insert('website_sections', sectionsToInsert);
    ...
    <div className="h-[calc(100vh-5rem)] flex flex-col bg-[#0A0A0A] overflow-hidden">
      ...
      <div className="h-16 border-b border-white/5 bg-[#111] px-4 md:px-6 flex items-center justify-between shrink-0 z-30">
        ...
        <div className="flex items-center gap-1 md:gap-3">
          ...
          <button className="p-2 md:px-4 md:py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-sm font-bold transition-all flex items-center gap-2 shadow-lg shadow-emerald-500/20">
