...
import {
  getWorkspaceViewTypePreference,
  isWorkspaceViewType,
  setWorkspaceViewTypePreference,
  type WorkspaceViewType,
} from "../../lib/workspaceViewPreference";
import {
  getWorkspaceTopbarTonePreference,
  isWorkspaceTopbarTone,
  setWorkspaceTopbarTonePreference,
  type WorkspaceTopbarTone,
} from "../../lib/workspaceTopbarPreference";
import {
  getStorefrontLayout,
  normalizeStorefrontThemeConfig,
} from "../../lib/storefrontLayoutPreference";
...
const TOPBAR_TONE_OPTIONS: Array<{
  id: WorkspaceTopbarTone;
  label: string;
  description: string;
  preview: string;
}> = [
  {
    id: "white",
    label: "White",
    description: "Clean pearl with dark controls.",
    preview: "from-white via-slate-50 to-slate-200",
  },
  {
    id: "dark",
    label: "Dark",
    description: "Graphite with soft depth.",
    preview: "from-[#18181b] via-[#101014] to-black",
  },
  {
    id: "indigo",
    label: "Indigo",
    description: "Focused midnight blue.",
    preview: "from-indigo-950 via-indigo-900 to-slate-950",
  },
  {
    id: "violet",
    label: "Violet",
    description: "Rich plum with a soft glow.",
    preview: "from-violet-950 via-purple-900 to-slate-950",
  },
  {
    id: "emerald",
    label: "Emerald",
    description: "Deep forest with fresh contrast.",
...
const getStoredThemePreference = (): ThemePreference => {
  const savedPreference = safeLocalStorage.getItem(
...
const fetchWorkspaceThemePreference = async (
  userId: string,
  ...
  try {
    ...
    return isThemePreference(data?.workspace_theme_preference)
      ? data.workspace_theme_preference
      : null;
  } catch {
    return null;
  }
...
const fetchWorkspaceViewTypePreference = async (
  userId: string,
): Promise<WorkspaceViewType | null> => {
  try {
    const { data, error } = await supabase
      .from("user_preferences")
      .select("workspace_view_type")
      .eq("user_id", userId)
      .maybeSingle();

    if (error) {
      console.error("Error loading workspace view type:", error);
      return null;
    }

    return isWorkspaceViewType(data?.workspace_view_type)
      ? data.workspace_view_type
      : null;
  } catch {
    return null;
  }
...
const fetchWorkspaceTopbarTonePreference = async (
  userId: string,
): Promise<WorkspaceTopbarTone | null> => {
  try {
    const { data, error } = await supabase
      .from("user_preferences")
      .select("workspace_topbar_tone")
      .eq("user_id", userId)
      .maybeSingle();

    if (error) {
      console.error("Error loading workspace topbar tone:", error);
      return null;
    }

    return isWorkspaceTopbarTone(data?.workspace_topbar_tone)
      ? data.workspace_topbar_tone
      : null;
  } catch {
    return null;
  }
...
export const AccountSettingsView = ({
  ...
  const [workspaceViewType, setWorkspaceViewType] = useState<WorkspaceViewType>(
    getWorkspaceViewTypePreference,
  );
  const [workspaceTopbarTone, setWorkspaceTopbarTone] =
    useState<WorkspaceTopbarTone>(getWorkspaceTopbarTonePreference);
  const [mobileView, setMobileView] = useState<"menu" | "detail">("menu");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [user, setUser] = useState<any>(null);
  const [profile, setProfile] = useState<any>(null);
  const [storefrontBusiness, setStorefrontBusiness] = useState<any>(null);
  const [storefrontFeedEnabled, setStorefrontFeedEnabled] = useState(false);
  const [savingStorefrontLayout, setSavingStorefrontLayout] = useState(false);
  ...
  const handleDownloadPreviousExport = async (exportItem: any) => {
    try {
      const result = await trustCenterAction<{ signedUrl: string }>(
        "download-export",
  ...
  const handleDeleteAccount = async () => {
    if (
      !(await destructiveAction({
        description:
          "Schedule account deletion with a 30-day cancellation period?",
      }))
    )
      return;
    try {
      ...
      appToast(
        error?.message || "The deletion request could not be scheduled.",
      );
    }
  };
  ...
  const handleGoogleCalendarSync = async () => {
    ...
    try {
      ...
      appToast(
        "Failed to sync Google Calendar: " + (error.message || "Unknown error"),
      );
      setSyncingCalendar(false);
    }
  };
  ...
  const fetchStripeAccountDetails = async () => {
    ...
    try {
      ...
      try {
        const balance = await invokeApiRunner("stripe-balance-get", {
          accountId,
        });
      ...
      setStripeBalance(null);
      setStripeInfoError(
        "We could not verify your Wersee Pay connection right now.",
      );
    } finally {
  ...
  const desktopApi: any =
    typeof window !== "undefined" ? (window as any).werseeDesktop : null;
    ...
    try {
      ...
      if (user) {
        const [
          ...
          remoteWorkspaceViewType,
          remoteWorkspaceTopbarTone,
          storefrontResult,
        ] = await Promise.all([
          supabase.from("profiles").select("*").eq("id", user.id).maybeSingle(),
          fetchWorkspaceThemePreference(user.id),
          fetchWorkspaceViewTypePreference(user.id),
          fetchWorkspaceTopbarTonePreference(user.id),
          supabase
            .from("businesses")
        ...
        if (storefrontResult.error) {
          console.error(
            "Error loading storefront appearance:",
            storefrontResult.error,
          );
        }
        if (storefrontResult.data) {
          setStorefrontBusiness(storefrontResult.data);
          setStorefrontFeedEnabled(
            getStorefrontLayout(storefrontResult.data.theme_config) === "feed",
          );
        } else {
          setStorefrontBusiness(null);
          setStorefrontFeedEnabled(false);
        }

        const storedTheme = getStoredThemePreference();
        const legacyProfileTheme = isThemePreference(profile?.theme)
          ? profile.theme
          : null;
        ...
        const nextWorkspaceViewType =
          remoteWorkspaceViewType || getWorkspaceViewTypePreference();
        setWorkspaceViewType(nextWorkspaceViewType);
        setWorkspaceViewTypePreference(nextWorkspaceViewType);
        const nextWorkspaceTopbarTone =
          remoteWorkspaceTopbarTone || getWorkspaceTopbarTonePreference();
        setWorkspaceTopbarTone(nextWorkspaceTopbarTone);
        setWorkspaceTopbarTonePreference(nextWorkspaceTopbarTone);
  ...
  const handleWorkspaceViewTypeChange = async (viewType: WorkspaceViewType) => {
    setWorkspaceViewType(viewType);
    setWorkspaceViewTypePreference(viewType);
    ...
    try {
      ...
      if (error) throw error;
      setSuccessMessage("Workspace view saved");
      window.setTimeout(() => setSuccessMessage(""), 2500);
    } catch (error) {
      console.error("Error saving workspace view type:", {
        message: (error as any)?.message,
        code: (error as any)?.code,
        details: (error as any)?.details,
      });
      setSuccessMessage("View changed locally. Cloud sync failed.");
      window.setTimeout(() => setSuccessMessage(""), 3000);
    }
  };

  const handleWorkspaceTopbarToneChange = async (tone: WorkspaceTopbarTone) => {
    setWorkspaceTopbarTone(tone);
    setWorkspaceTopbarTonePreference(tone);
    ...
    try {
      ...
      if (error) throw error;
      setSuccessMessage("Topbar color saved");
      window.setTimeout(() => setSuccessMessage(""), 2500);
    } catch (error) {
      console.error("Error saving workspace topbar tone:", {
        message: (error as any)?.message,
        code: (error as any)?.code,
        details: (error as any)?.details,
      });
      setSuccessMessage("Color changed locally. Cloud sync failed.");
      window.setTimeout(() => setSuccessMessage(""), 3000);
    }
  };

  const handleStorefrontFeedChange = async (enabled: boolean) => {
    if (!user || !storefrontBusiness || savingStorefrontLayout) return;

    const previousEnabled = storefrontFeedEnabled;
    const nextThemeConfig = {
      ...normalizeStorefrontThemeConfig(storefrontBusiness.theme_config),
      storefront_layout: enabled ? "feed" : "classic",
    };

    setStorefrontFeedEnabled(enabled);
    setSavingStorefrontLayout(true);
    setSuccessMessage("");
    try {
      const { data, error } = await supabase
        .from("businesses")
        .update({ theme_config: nextThemeConfig })
        .eq("id", storefrontBusiness.id)
        .eq("user_id", user.id)
        .select("id, slug, name, theme_config")
        .maybeSingle();

      if (error) throw error;
      if (!data) throw new Error("The storefront setting was not saved.");

      setStorefrontBusiness(data);
      setStorefrontFeedEnabled(
        getStorefrontLayout(data.theme_config) === "feed",
      );
      setSuccessMessage(
        enabled ? "Storefront feed enabled" : "Classic storefront restored",
      );
      window.setTimeout(() => setSuccessMessage(""), 3000);
    } catch (error) {
      setStorefrontFeedEnabled(previousEnabled);
      console.error("Error saving storefront feed layout:", {
        message: (error as any)?.message,
        code: (error as any)?.code,
        details: (error as any)?.details,
      });
      appToast("Your storefront layout could not be saved.");
    } finally {
      setSavingStorefrontLayout(false);
    }
  };
  ...
  const handleTerminateAllOtherSessions = async () => {
    if (
      !(await destructiveAction({
        description:
          "Are you sure you want to log out all other devices? You will need to log in again.",
      }))
    )
      return;
  ...
  const stripeStatusItems = [
    {
      label: "Payments",
      value: stripeAccountInfo?.charges_enabled ? "Enabled" : "Pending",
    },
    {
      label: "Payouts",
      value: stripeAccountInfo?.payouts_enabled ? "Enabled" : "Pending",
    },
    {
      label: "Country",
      value: String(stripeAccountInfo?.country || "—").toUpperCase(),
    },
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row bg-transparent min-h-[100dvh]">
      ...
      <div
        className={`md:hidden ${mobileView === "menu" ? "block" : "hidden"} flex-1 bg-transparent min-h-[100dvh]`}
      >
        <div
          className={`px-4 py-6 sticky top-0 ${isDark ? "bg-black/80" : "bg-white/80"} backdrop-blur-xl z-10 border-b border-white/5`}
        >
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`p-2.5 rounded-xl ${tab.danger ? "bg-red-500/10" : "bg-white/5"}`}
                  >
                ...
                <ChevronRight
                  className={`w-5 h-5 ${tab.danger ? "text-red-400/50" : "text-gray-500"}`}
                />
              </button>
            );
          })}
      ...
      {/* Settings Content */}
      <div
        className={`flex-1 overflow-y-auto p-4 md:p-8 pb-32 md:pb-8 ${mobileView === "detail" ? "block" : "hidden md:block"}`}
      >
          ...
          <div className="space-y-8">
            ...
            {activeTab === "appearance" && (
              <div className="space-y-6">
                <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-indigo-500/15 via-purple-500/5 to-transparent p-5 md:p-8">
                  ...
                  <div className="relative max-w-xl">
                    ...
                    <p className="mt-2 text-sm leading-relaxed text-gray-400">
                      Your choice applies instantly across navigation,
                      dashboards, settings, and workspace tools, and follows you
                      on your next sign-in.
                    </p>
                  </div>
                </section>

                <section className="relative overflow-hidden rounded-[2rem] border border-white/5 bg-[#141414] p-4 md:p-6">
                  <div className="pointer-events-none absolute -right-20 top-4 h-44 w-44 rounded-full bg-fuchsia-500/10 blur-3xl" />
                  <div className="relative mb-5 flex items-start gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-fuchsia-500/10 text-fuchsia-200 ring-1 ring-fuchsia-400/15">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm leading-6 text-gray-500">
                        Give your public store its own navigation and a
                        cinematic, continuously scrolling release feed.
                      </p>
                    </div>
                  </div>

                  {storefrontBusiness ? (
                    <div className="relative grid gap-5 rounded-[26px] border border-white/[0.07] bg-[#09090b] p-3 md:grid-cols-[1.15fr_0.85fr] md:p-4">
                      <div className="relative h-52 overflow-hidden rounded-[20px] border border-white/[0.06] bg-[radial-gradient(circle_at_72%_22%,rgba(168,85,247,0.2),transparent_36%),linear-gradient(145deg,#111116,#070708)]">
                        <span className="absolute bottom-3 left-3 top-3 flex w-16 flex-col rounded-2xl border border-white/[0.06] bg-black/35 p-2 backdrop-blur-lg">
                          <span className="h-8 w-8 rounded-xl bg-white/90" />
                          <span className="mt-5 h-2 w-10 rounded-full bg-fuchsia-300/55" />
                          <span className="mt-3 h-2 w-8 rounded-full bg-white/13" />
                          <span className="mt-3 h-2 w-9 rounded-full bg-white/13" />
                          <span className="mt-auto h-8 rounded-xl border border-white/[0.06] bg-white/[0.04]" />
                        </span>
                        <span className="absolute bottom-3 left-[88px] right-3 top-3 overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.025] p-3">
                          <span className="block h-16 rounded-xl bg-gradient-to-br from-fuchsia-400/20 to-indigo-400/5" />
                          <span className="mt-3 block h-3 w-3/5 rounded-full bg-white/24" />
                          <span className="mt-2 block h-2 w-4/5 rounded-full bg-white/10" />
                          <span className="mt-4 block h-12 rounded-xl bg-white/[0.055]" />
                          <span className="mt-3 block h-2 w-1/2 rounded-full bg-white/10" />
                        </span>
                        <span className="absolute bottom-4 right-4 rounded-full border border-white/10 bg-black/55 px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.18em] text-white/65 backdrop-blur-md">
                          Feed preview
                        </span>
                      </div>

                      <div className="flex flex-col justify-between px-1 py-1 md:py-2">
                        <div>
                          <div className="flex items-center gap-2 text-xs font-semibold text-fuchsia-200/75">
                            <Eye className="h-4 w-4" /> Public layout
                          </div>
                          <h5 className="mt-3 text-lg font-bold tracking-[-0.025em] text-white">
                            Sidebar feed
                          </h5>
                          <p className="mt-2 text-sm leading-6 text-gray-500">
                            Replaces the standard Wersee header on{" "}
                            {storefrontBusiness.name} with store-branded
                            sections, editorial product cards, and real scroll
                            loading.
                          </p>
                        </div>
                        ...
                        <div className="mt-5 space-y-3">
                          <button
                            type="button"
                            role="switch"
                            aria-checked={storefrontFeedEnabled}
                            disabled={savingStorefrontLayout}
                            onClick={() =>
                              void handleStorefrontFeedChange(
                                !storefrontFeedEnabled,
                              )
                            }
                            className="flex min-h-12 w-full items-center justify-between gap-4 rounded-2xl border border-white/[0.07] bg-white/[0.035] px-4 text-left transition hover:bg-white/[0.06] disabled:cursor-wait disabled:opacity-60"
                          >
                            <span>
                              <span className="block text-sm font-semibold text-white">
                                {storefrontFeedEnabled
                                  ? "Feed is on"
                                  : "Use classic storefront"}
                              </span>
                              <span className="mt-0.5 block text-[11px] text-white/32">
                                Saved to your public storefront
                              </span>
                            </span>
                            <span
                              className={`relative h-7 w-12 shrink-0 rounded-full border transition ${storefrontFeedEnabled ? "border-fuchsia-300/35 bg-fuchsia-500" : "border-white/10 bg-white/10"}`}
                            >
                              <span
                                className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow-lg transition-transform ${storefrontFeedEnabled ? "translate-x-6" : "translate-x-1"}`}
                              />
                            </span>
                          </button>

                          {storefrontBusiness.slug ? (
                            <a
                              href={`/${storefrontBusiness.slug}`}
                              target="_blank"
                              rel="noreferrer"
                              className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-2xl text-sm font-semibold text-white/45 transition hover:bg-white/[0.035] hover:text-white"
                            >
                    ...
                    <div className="relative rounded-[24px] border border-dashed border-white/10 bg-white/[0.02] p-6 text-center">
                      <Store className="mx-auto h-6 w-6 text-white/20" />
                      <p className="mt-3 text-sm font-semibold text-white/72">
                        No storefront available
                      </p>
                      <p className="mt-1 text-xs leading-5 text-white/32">
                        Create a business storefront before choosing a public
                        feed layout.
                      </p>
                    </div>
                  )}
                ...
                <section className="rounded-[2rem] border border-white/5 bg-[#141414] p-4 md:p-6">
                  <div className="mb-5">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Choose the contrast that feels best for your work.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    {THEME_OPTIONS.map((option) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`relative mb-4 block h-28 overflow-hidden rounded-2xl bg-gradient-to-br ${previewClass} ring-1 ring-black/5`}
                          >
                          ...
                          <span className="flex items-center justify-between gap-3 px-1">
                            ...
                            <span
                              className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${isSelected ? "border-indigo-500 bg-indigo-500" : "border-white/20"}`}
                            >
                              {isSelected && (
                ...
                <section className="rounded-[2rem] border border-white/5 bg-[#141414] p-4 md:p-6">
                  <div className="mb-5">
                    <h4 className="font-bold text-white">Workspace view</h4>
                    <p className="mt-1 text-sm text-gray-500">
                      Choose the new full-width command bar or keep the classic
                      icon sidebar.
                    </p>
                  </div>
                    ...
                    {[
                      {
                        ...
                        label: "Topbar",
                        description:
                          "A compact Shopify-style command bar with more room for your work.",
                        icon: PanelTop,
                      },
                      {
                        id: "sidebar" as const,
                        ...
                        description:
                          "The current Wersee icon rail with expanding navigation.",
                        icon: PanelLeft,
                      },
                    ].map((option) => {
                      const Icon = option.icon;
                      const isSelected = workspaceViewType === option.id;
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            handleWorkspaceViewTypeChange(option.id)
                          }
                          aria-pressed={isSelected}
                          className={`group rounded-3xl border p-3 text-left transition-all focus-visible:outline-none ${
                            isSelected
                              ? "border-violet-400/70 bg-violet-500/10 shadow-[0_14px_35px_rgba(139,92,246,0.14)]"
                              : "border-white/10 bg-white/[0.02] hover:-translate-y-0.5 hover:border-white/20"
                          }`}
                        >
                          <span className="relative mb-4 block h-32 overflow-hidden rounded-2xl border border-white/5 bg-[#09090b]">
                            {option.id === "topbar" ? (
                              <>
                                <span className="absolute inset-x-2 top-2 flex h-8 items-center gap-1.5 rounded-xl bg-white/[0.08] px-2">
                                  <span className="h-4 w-4 rounded-md bg-white" />
                                  <span className="h-4 w-4 rounded-full bg-violet-400/35" />
                                  <span className="h-4 w-16 rounded-full bg-white/10" />
                                  <span className="ml-auto h-4 w-20 rounded-full bg-white/10" />
                                  <span className="h-4 w-4 rounded-full bg-white/20" />
                                </span>
                                <span className="absolute inset-x-2 bottom-2 top-12 grid grid-cols-3 gap-2 rounded-xl bg-white/[0.025] p-2">
                                  <span className="col-span-2 rounded-lg bg-violet-400/15" />
                                  <span className="rounded-lg bg-white/[0.07]" />
                                </span>
                              </>
                            ) : (
                              <>
                                <span className="absolute bottom-2 left-2 top-2 flex w-8 flex-col items-center gap-2 rounded-xl bg-white/[0.08] py-2">
                                  <span className="h-4 w-4 rounded-md bg-white" />
                                  <span className="h-3 w-3 rounded-full bg-violet-400/40" />
                                  <span className="h-3 w-3 rounded-full bg-white/15" />
                                  <span className="h-3 w-3 rounded-full bg-white/15" />
                                </span>
                                <span className="absolute bottom-2 left-12 right-2 top-2 grid grid-cols-2 gap-2 rounded-xl bg-white/[0.025] p-2">
                                  <span className="col-span-2 h-6 rounded-lg bg-white/[0.07]" />
                                  <span className="rounded-lg bg-violet-400/15" />
                                  <span className="rounded-lg bg-white/[0.07]" />
                                </span>
                              </>
                            )}
                          ...
                          <span className="flex items-center justify-between gap-3 px-1">
                            ...
                            <span
                              className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${isSelected ? "border-violet-500 bg-violet-500" : "border-white/20"}`}
                            >
                              {isSelected ? (
                                <CheckCircle2 className="h-3.5 w-3.5 text-white" />
                              ) : null}
                ...
                <section className="rounded-[2rem] border border-white/5 bg-[#141414] p-4 md:p-6">
                  <div className="mb-5 flex items-start gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-violet-500/10 text-violet-300 ring-1 ring-violet-400/15">
                      <Palette className="h-5 w-5" />
                    </span>
                    <div>
                      <h4 className="font-bold text-white">Topbar color</h4>
                      <p className="mt-1 text-sm text-gray-500">
                        Give the topbar its own surface. This applies to the
                  ...
                  <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
                    {TOPBAR_TONE_OPTIONS.map((option) => {
                      const isSelected = workspaceTopbarTone === option.id;
                      const usesDarkInk = option.id === "white";
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            handleWorkspaceTopbarToneChange(option.id)
                          }
                          aria-pressed={isSelected}
                          className={`group rounded-3xl border p-2.5 text-left transition-all focus-visible:outline-none ${
                            isSelected
                              ? "border-violet-400/70 bg-violet-500/10 shadow-[0_12px_30px_rgba(139,92,246,0.13)]"
                              : "border-white/10 bg-white/[0.02] hover:-translate-y-0.5 hover:border-white/20"
                          }`}
                        >
                          <span
                            className={`relative mb-3 block h-20 overflow-hidden rounded-2xl bg-gradient-to-br ${option.preview} ring-1 ring-white/10`}
                          >
                            <span
                              className={`absolute inset-x-2 top-2 flex h-7 items-center gap-1 rounded-xl px-2 shadow-sm ${
                                usesDarkInk
                                  ? "bg-black/[0.055]"
                                  : "bg-white/[0.09]"
                              }`}
                            >
                              <span
                                className={`h-3.5 w-3.5 rounded-md ${usesDarkInk ? "bg-slate-900" : "bg-white"}`}
                              />
                              <span
                                className={`h-2 w-7 rounded-full ${usesDarkInk ? "bg-slate-900/20" : "bg-white/20"}`}
                              />
                              <span
                                className={`ml-auto h-3.5 w-8 rounded-full ${usesDarkInk ? "bg-slate-900/10" : "bg-white/15"}`}
                              />
                            </span>
                            <span
                              className={`absolute inset-x-2 bottom-2 top-12 rounded-xl ${usesDarkInk ? "bg-white/55" : "bg-black/15"}`}
                            />
                          </span>
                          <span className="flex items-center justify-between gap-2 px-1">
                            <span className="min-w-0">
                              ...
                              <span className="mt-1 hidden text-[10px] leading-snug text-gray-500 xl:block">
                                {option.description}
                              </span>
                            </span>
                            <span
                ...
                <div className="flex items-start gap-3 rounded-2xl border border-emerald-500/15 bg-emerald-500/5 p-4 text-sm text-gray-400">
                  ...
                  <p>
                    {successMessage ||
                      "Appearance is saved automatically. No page refresh is needed."}
                  </p>
                </div>
              </div>
            )}
                ...
                <div className="bg-[#141414] border border-white/5 rounded-2xl p-6">
                  ...
                  {fetchingSessions ? (
                    ...
                    <div className="text-center py-8 text-gray-500">
                      <p>
                        No active sessions found or session tracking is
                        unavailable.
                      </p>
            ...
            {/* PASSKEYS TAB */}
            {activeTab === "passkeys" && <PasskeysSettings user={user} />}
                  ...
                  <div className="space-y-6">
                      ...
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              privacy_show_profile: e.target.checked,
                            })
                          }
                        />
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-400">
                          Allow us to collect usage data to improve Wersee.
                        </p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          className="sr-only peer"
                          checked={formData.privacy_allow_tracking}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              privacy_allow_tracking: e.target.checked,
                            })
                          }
                        />
                        <div className="w-11 h-6 bg-gray-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                ...
                <div className="bg-[#141414] border border-white/5 rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">
                        Download a copy of your data or view previous exports.
                      </p>
                    </div>
                      ...
                      {interactiveHistory.length > 0 ? (
                        <div className="grid grid-cols-1 gap-3">
                          {interactiveHistory.map((session) => (
                            <div
                              key={session.id}
                              className="flex items-center justify-between p-4 bg-blue-500/5 rounded-xl border border-blue-500/10"
                            >
                              <div className="flex items-center gap-4">
                        ...
                        <div className="text-center py-6 bg-white/5 rounded-xl border border-dashed border-white/10">
                          <p className="text-sm text-gray-500">
                            No interactive sites generated yet.
                          </p>
                        </div>
                      )}
                    </div>
                        ...
                        <div className="space-y-3">
                          {exportHistory.map((exp) => (
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  handleDownloadPreviousExport(exp)
                                }
                ...
                <div className="bg-[#141414] border border-white/5 rounded-2xl p-6">
                  ...
                  <div className="space-y-4">
                    <a
                      href="/terms"
                      target="_blank"
                      className="flex items-center justify-between p-4 bg-white/5 hover:bg-white/10 rounded-xl border border-white/5 transition-colors group"
                    >
                ...
                <section className="relative overflow-hidden rounded-[2rem] border border-indigo-500/20 bg-gradient-to-br from-indigo-600/20 via-purple-600/10 to-transparent p-5 md:p-8">
                  ...
                  <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                    <div className="max-w-xl">
                      ...
                      <p className="mt-2 text-sm leading-relaxed text-gray-400">
                        Connect and manage payouts securely through Wersee Pay.
                        Payment credentials are managed server-side, so you
                        never need to paste API keys into the workspace.
                      </p>
                    </div>
                    <div
                    ...
                    >
                      <span
                        className={`h-2 w-2 rounded-full ${stripeReady ? "bg-emerald-400" : stripeAccountId ? "bg-amber-400" : "bg-gray-500"}`}
                      />
                      {stripeReady
                        ? "Ready for payments"
                        : stripeAccountId
                ...
                {fetchingStripeInfo ? (
                  <section className="flex min-h-56 items-center justify-center rounded-[2rem] border border-white/5 bg-[#141414]">
                    <div className="text-center">
                      ...
                      <p className="mt-3 text-sm text-gray-500">
                        Checking your secure connection…
                      </p>
                    </div>
                  </section>
                ) : stripeAccountInfo ? (
                  <section className="rounded-[2rem] border border-white/5 bg-[#141414] p-5 md:p-6">
                    <div className="flex flex-col gap-4 border-b border-white/5 pb-5 sm:flex-row sm:items-center sm:justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`flex h-11 w-11 items-center justify-center rounded-2xl ${stripeReady ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-400"}`}
                        >
                          {stripeReady ? (
                        ...
                        <div>
                          <h4 className="font-bold text-white">
                            {stripeReady
                              ? "Wersee Pay is active"
                              : "Finish your Wersee Pay setup"}
                          </h4>
                          <p className="mt-0.5 text-xs text-gray-500">
                            Live status from your connected payment account
                          </p>
                        </div>
                      </div>
                      <button
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <p className="mt-1 max-w-md text-sm leading-relaxed text-gray-500">
                            Complete the hosted onboarding flow to accept
                            payments and receive payouts. No developer setup or
                ...
                <div className="flex items-start gap-3 rounded-2xl border border-emerald-500/15 bg-emerald-500/5 p-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-relaxed text-gray-500">
                      Wersee Pay uses the platform's managed Stripe connection.
                      Secret keys are never requested, displayed, or stored in
                      this browser.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-2 rounded-2xl border border-red-500/20 bg-red-500/10 p-4 text-sm text-red-400">
                    ...
                    <div className="flex-1">
                      ...
                      <button
                        type="button"
                        onClick={fetchStripeAccountDetails}
                        className="mt-2 font-bold text-red-300 hover:text-white"
                      >
                        Try again
                      </button>
                    </div>
                  ...
                  {[
                    {
                      ...
                      desc: "Open Wersee automatically when you sign in to your computer.",
                    },
                    {
                      k: "startMinimized",
                      label: "Start minimized to tray",
                      desc: "Start Wersee in the background without opening a window.",
                    },
                    {
                      k: "minimizeToTray",
                      label: "Close to system tray",
                      desc: "Keep Wersee running in the tray when you close the window.",
                    },
                  ].map((opt) => (
                    <div
                      key={opt.k}
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          updatePcSetting(opt.k, !pcSettings[opt.k])
                        }
                      ...
                      >
                        <span
                          className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${pcSettings[opt.k] ? "translate-x-5" : ""}`}
                        />
                      </button>
                    </div>
                  ))}
                ...
                <div className="bg-[#141414] border border-white/5 rounded-2xl p-4 md:p-6">
                  ...
                  <div className="flex items-start justify-between gap-4 py-4 border-b border-white/5">
                    <div className="min-w-0">
                      ...
                      <p className="text-gray-500 text-xs mt-1">
                        Use the GPU for smoother rendering. Turn off if you see
                        graphical glitches. Requires restart.
                      </p>
                    </div>
                    <button
                      onClick={() =>
                        updatePcSetting(
                          ...
                          !pcSettings.hardwareAcceleration,
                        )
                      }
                    ...
                    >
                      <span
                        className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${pcSettings.hardwareAcceleration ? "translate-x-5" : ""}`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="py-4 border-b border-white/5">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        ...
                        <p className="text-gray-500 text-xs mt-1">
                          Scale the interface text and UI.
                        </p>
                      </div>
                      <span className="text-indigo-400 font-mono text-sm">
                        {Math.round(pcSettings.zoomLevel * 100)}%
                      </span>
                    </div>
                    <input
                      type="range"
                      ...
                      onChange={(e) =>
                        updatePcSetting("zoomLevel", parseFloat(e.target.value))
                      }
                  ...
                  <div className="flex items-start justify-between gap-4 py-4 border-b border-white/5">
                    <div className="min-w-0">
                      ...
                      <p className="text-gray-500 text-xs mt-1">
                        Show system notifications for messages, orders and
                        payments.
                      </p>
                    </div>
                    <button
                      onClick={() =>
                        updatePcSetting(
                          ...
                          !pcSettings.notificationsEnabled,
                        )
                      }
                    ...
                    >
                      <span
                        className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${pcSettings.notificationsEnabled ? "translate-x-5" : ""}`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="flex items-start justify-between gap-4 py-4">
                    <div className="min-w-0">
                      ...
                      <p className="text-gray-500 text-xs mt-1">
                        Download and install new Wersee releases in the
                        background.
                      </p>
                    </div>
                    <button
                      onClick={() =>
                        updatePcSetting("autoUpdate", !pcSettings.autoUpdate)
                      }
                    ...
                    >
                      <span
                        className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${pcSettings.autoUpdate ? "translate-x-5" : ""}`}
                      />
                    </button>
                  </div>
                </div>
            ...
            {activeTab === "developer" && (
              <div className="space-y-6">
                ...
                <div className="bg-[#141414] border border-white/5 rounded-2xl p-4 md:p-6">
                  ...
                  <p className="text-gray-400 text-sm mb-6">
                    Manage your API keys and developer access to integrate
                    Wersee with your own applications.
                  </p>
              ...
              <div className="space-y-6">
                <div className="bg-[#141414] border border-white/5 rounded-2xl p-4 md:p-6">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <h3 className="text-lg font-bold text-white">
                      10-Year Historical Lookup
                    </h3>
                  </div>
                  <p className="text-gray-400 text-sm mb-6">
                    Access your complete historical archive. We retain your data
                    for up to 10 years for compliance and personal
                    record-keeping. Links in this archive remain active for 6
                    months.
                  </p>
                  <div className="space-y-4 mb-6">
                    {exportHistory.length > 0 ? (
                            ...
                            <div className="flex items-center gap-3 md:gap-4 overflow-hidden">
                              ...
                              <div className="min-w-0">
                                <p className="text-white font-medium text-sm truncate">
                                  Historical Archive (
                                  {format(new Date(exp.created_at), "yyyy")})
                                </p>
                                <p className="text-xs text-gray-500">
                                  Generated{" "}
                      ...
                      <div className="text-center py-8 bg-black/40 rounded-2xl border border-dashed border-white/5">
                        <p className="text-sm text-gray-500">
                          No historical records found for the last 10 years.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    ...
                    {downloadingData
                      ? "Generating Full Archive..."
                      : "Generate 10-Year Data Export"}
                  </button>
                </div>
            ...
            {activeTab === "danger" && (
              <div className="rounded-3xl border border-red-400/15 bg-red-400/[0.04] p-6">
                <h2 className="text-lg font-black text-white">
                  Privacy and account closure
                </h2>
                <p className="mt-2 max-w-xl text-sm leading-6 text-white/45">
                  Exports and account deletion now use a verified, auditable
                  workflow with a cancellation period and separate handling for
