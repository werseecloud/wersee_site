...
const compileUploadedApp = async (file: File) => {
  if (file.size > 10 * 1024 * 1024) throw new Error('APP_PACKAGE_TOO_LARGE');
  const zip = await JSZip.loadAsync(file);
  const indexEntry = Object.values(zip.files)
    .filter((entry) => !entry.dir && /(^|\/)index\.html$/i.test(entry.name))
    .sort((left, right) => left.name.length - right.name.length)[0];
  if (!indexEntry) throw new Error('APP_INDEX_MISSING');
  const html = await indexEntry.async('string');
  if (html.length > 2_000_000) throw new Error('APP_HTML_TOO_LARGE');

  const documentNode = new DOMParser().parseFromString(html, 'text/html');
  const localReferences = Array.from(documentNode.querySelectorAll('[src], link[href]'))
    .map((element) => element.getAttribute('src') || element.getAttribute('href') || '')
    .filter((value) => value && !/^(data:|blob:|https?:|#)/i.test(value));
  if (localReferences.length > 0) throw new Error('APP_PACKAGE_NOT_SELF_CONTAINED');
...
export const AppBuilder: React.FC<AppBuilderProps> = ({ businessId, onClose, onSave, existingApp }) => {
  ...
  const previewUrl = appSlug
    ? `wersee.com${routes.appById({ appId: appSlug })}`
    : 'wersee.com/app/[app-id]';

  const handleNext = () => {
    if (step === 2 && buildMode === 'upload' && !distFile) {
  ...
  const handleSave = async (retries = 3) => {
    ...
    if (!appSlug) {
      appToast('Use a name with at least one letter or number.');
      return;
    }
    ...
    try {
      ...
      const compiledCode = buildMode === 'upload'
        ? await compileUploadedApp(distFile as File)
        : manualCode;
      ...
      onSave();
      appToast(existingApp ? 'App updated.' : 'App created.', 'success');
    } catch (error: any) {
      ...
      const messages: Record<string, string> = {
        APP_PACKAGE_TOO_LARGE: 'The ZIP package is larger than 10 MB.',
        APP_INDEX_MISSING: 'The ZIP package needs an index.html file.',
        APP_HTML_TOO_LARGE: 'The compiled index.html is too large.',
        APP_PACKAGE_NOT_SELF_CONTAINED: 'This ZIP still references local files. Export it as one self-contained index.html and try again.',
      };
      appToast(messages[error?.message] || (error?.code === '23505' ? 'That app name or slug is already in use.' : 'Failed to save app.'), 'error');
    } finally {
      setIsSaving(false);
    }
  };
                  ...
                  <>
                    <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-2xl p-4 sm:p-6 mb-6">
                      ...
                      <p className="text-xs sm:text-sm text-indigo-200/70 leading-relaxed">
                        Upload a compiled ZIP containing one self-contained index.html. External HTTPS resources are supported; local asset files must be bundled into the HTML.
                      </p>
                    </div>
