...
import { supabase, invokeApiRunner, invokeFinanceWorkflow } from '../../lib/supabase';
...
export const MoneyInvoicesView = () => {
    ...
    try {
      ...
      if (!dbError && insertedInvoice) {
        const paymentLink = werseePaymentUrls.invoice({
          username,
          invoiceId: insertedInvoice.invoice_number || formData.invoice_number,
          sandbox: insertedInvoice.stripe_invoice_id === 'sandbox_invoice',
        });
        await supabase
          .from('invoice_links')
      ...
      let paymentLink = werseePaymentUrls.invoice({
        username,
        invoiceId: insertedInvoice?.invoice_number || formData.invoice_number,
        sandbox: insertedInvoice?.stripe_invoice_id === 'sandbox_invoice',
      });
  ...
  const handleSendEmail = async (invoiceToUse?: any) => {
    ...
    try {
      await invokeFinanceWorkflow('invoice-send', { invoiceId: invoice.id });
      appToast(`Invoice sent from invoice@wersee.com to ${invoice.customer_email || recipientEmail}`);
      setEmailSent(true);
      ...
      const paymentLink = werseePaymentUrls.invoice({
        ...
        invoiceId: invoice.invoice_number || formData.invoice_number,
        sandbox: invoice.stripe_invoice_id === 'sandbox_invoice',
      });
      ...
      appToast('Invoice delivery is handled by the secure finance email outbox.');
                ...
                html: `
                  <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #E5E7EB; border-radius: 16px;">
                    ...
                    <div style="background-color: #F9FAFB; padding: 24px; border-radius: 12px; margin-bottom: 32px;">
                      ...
                      <a href="${werseePaymentUrls.invoice({ username: invoice.username, invoiceId: invoice.invoice_number, sandbox: invoice.stripe_invoice_id === 'sandbox_invoice' })}" style="display: block; width: 100%; padding: 14px 0; background-color: #635BFF; color: white; text-align: center; text-decoration: none; border-radius: 10px; font-weight: bold; margin-bottom: 12px; font-size: 16px;">
                        Pay Invoice Now
                      </a>
                      <a href="${werseePaymentUrls.invoice({ username: invoice.username, invoiceId: invoice.invoice_number, sandbox: invoice.stripe_invoice_id === 'sandbox_invoice' })}?download=true" style="display: block; width: 100%; padding: 14px 0; background-color: white; color: #111827; text-align: center; text-decoration: none; border-radius: 10px; font-weight: bold; border: 1px solid #E5E7EB; font-size: 16px;">
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      Return only the final JSON object. Do not include markdown, commentary, hidden reasoning, or chain-of-thought.`;
      ...
      const jsonText = fullResponse.replace(/```json/gi, '').replace(/```/g, '').trim();
      if (jsonText) {
        const jsonData = JSON.parse(jsonText);
        setReasoningSteps(INITIAL_REASONING_STEPS.map((step) => ({ ...step, status: 'completed', content: '' })));
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-mono text-sm truncate">
                          {werseePaymentUrls.invoice({ username: createdInvoice.username, invoiceId: createdInvoice.invoice_number, sandbox: createdInvoice.stripe_invoice_id === 'sandbox_invoice' })}
                        </div>
                        <button 
                          onClick={() => {
                            navigator.clipboard.writeText(werseePaymentUrls.invoice({ username: createdInvoice.username, invoiceId: createdInvoice.invoice_number, sandbox: createdInvoice.stripe_invoice_id === 'sandbox_invoice' }));
                            setCopied(true);
                      ...
                      <div className="w-full flex justify-center">
                        <div className={`origin-top shadow-2xl rounded-[1rem] overflow-hidden bg-white transition-all duration-500 ${
                          ...
                          <InvoicePDFPreview 
                            ...
                            isHidden={false} 
                            paymentLink={createdInvoice ? werseePaymentUrls.invoice({ username: createdInvoice.username, invoiceId: createdInvoice.invoice_number, sandbox: createdInvoice.stripe_invoice_id === 'sandbox_invoice' }) : undefined}
                          />
                        </div>
                      </div>
                    )}
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const excelData = invoices.map(inv => ({
                      ...
                      'Payment Link': werseePaymentUrls.invoice({ username: inv.username, invoiceId: inv.invoice_number, sandbox: inv.stripe_invoice_id === 'sandbox_invoice' })
                    }));
                    ...
                    const escapeCsvCell = (value: unknown) => {
                      ...
                      // Prevent spreadsheet formula injection when customer-controlled values are opened in Excel.
                      const safe = /^[=+\-@\t\r]/.test(raw) ? `'${raw}` : raw;
                      return `"${safe.replace(/"/g, '""')}"`;
                    };
                    const csvRows = [
                      csvHeaders.map(escapeCsvCell).join(','),
                      ...excelData.map(row => csvHeaders.map(header => escapeCsvCell(row[header])).join(','))
                    ];
                    zip.file("invoices_summary.csv", `\uFEFF${csvRows.join('\r\n')}`);
