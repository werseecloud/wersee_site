...
export const MoneyPosView: React.FC<MoneyPosViewProps> = ({ isStripeComplete }) => {
  ...
  const [profileUsername, setProfileUsername] = useState<ReturnType<typeof parseUsername>>(null);
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      const { data: profile } = await supabase
        ...
        .eq('id', user.id)
        .maybeSingle();
      setProfileUsername(parseUsername(profile?.username));
  ...
  const handleLaunchTerminal = () => {
    ...
    window.open(routes.userPosTerminal({ username: profileUsername, systemName }), '_blank');
  };
