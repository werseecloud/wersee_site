...
export const FormsManagementView = () => {
  ...
  const fetchForms = async () => {
    ...
    try {
      ...
      const uniqueForms = Array.from(new Map((data || []).map((f: any) => [f.id, f])).values()) as Form[];
  ...
  const handleCreateForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { data, error } = await DatabaseService.insert('forms', { 
        name, 
        slug, 
        user_id: user?.id,
      ...
      if (error) throw error;
      const newForm = Array.isArray(data) ? data[0] : data;
      setFunnels([newForm, ...forms]);
      setIsCreating(false);
      setSelectedForm(newForm);
    } catch (error) {
      console.error('Error creating form:', error);
      appToast('Error creating form. Slug might be taken.');
    }
  };
  ...
  const handleSaveForm = async () => {
    ...
    setSaving(true);
    try {
      const { error } = await DatabaseService.update('forms', selectedForm.id, {
      ...
      appToast('Form saved successfully!');
    } catch (error) {
