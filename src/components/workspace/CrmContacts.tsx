...
export const CrmContacts = ({ listingId }: { listingId?: string }) => {
  ...
  const handleSaveContact = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedContact) {
        await DatabaseService.update('crm_contacts', selectedContact.id, contactData);
        ...
        const newContact = await DatabaseService.insert<any>('crm_contacts', contactData);
      ...
      setSaveState('error');
      appToast(error instanceof Error ? error.message : 'Failed to save contact');
    }
  };
            ...
            >
              ...
              <div className="p-6 border-t border-white/5 flex items-center justify-end gap-3 shrink-0 bg-[#0A0A0A]">
                ...
                <SaveStateButton
                  ...
                  form="contact-form"
                  state={saveState}
                  idleLabel={selectedContact ? 'Save Changes' : 'Add Contact'}
                  savedLabel="Contact saved"
                />
              </div>
            </motion.div>
          </div>
