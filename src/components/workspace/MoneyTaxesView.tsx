...
import type { TaxAnalysisResponse } from '../../services/taxAiService';
...
export const MoneyTaxesView = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setAnalyzing(true);
      const result = await invokeFinanceWorkflow<any>('tax-summary');
      setReportYear(result.year);
      setTrendPercent(typeof result.trendPercent === 'number' ? result.trendPercent : null);
      setStats({
        grossRevenue: Number(result.currentRevenue || 0),
        stripeFees: Number(result.stripeFees || 0),
        werseeFees: Number(result.werseeFees || 0),
        totalTransactions: Number(result.transactionCount || 0),
      });
      setTransactions(result.transactions || []);
      const taxReserve = Number(result.taxAmount || 0);
      setTaxData({
        vatRate: result.currentRevenue > 0 ? taxReserve / result.currentRevenue : 0,
        vatAmount: taxReserve,
        netProfit: Number(result.netProfit || 0),
        taxReserve,
        ossStatus: {
          thresholdReached: Number(result.currentRevenue || 0) > 10000,
          currentAmount: Number(result.currentRevenue || 0),
          limit: 10000,
        },
        nexusStatus: { hasNexus: false, states: [] },
        summary: result.transactionCount
          ? `${result.transactionCount} payment(s) for ${result.year} matched exactly between Stripe and Supabase. Unmatched rows are excluded.`
          : `No Stripe and Supabase payments matched for ${result.year}; amounts remain neutral at zero.`,
      });
    } catch (error) {
  ...
  const downloadBlob = (name: string, type: string, content: string | Blob) => {
  ...
  const exportCsv = () => {
    const rows = [['id', 'created_at', 'amount', 'currency', 'stripe_fee', 'wersee_fee', 'tax_amount'], ...transactions.map((row) => [
      row.id, row.created_at, row.amount, row.currency, row.stripe_fee, row.wersee_fee, row.tax_amount,
    ])];
    downloadBlob(`wersee-tax-${reportYear}.csv`, 'text/csv;charset=utf-8', rows.map((row) => row.map((value) => `"${String(value ?? '').replace(/"/g, '""')}"`).join(',')).join('\n'));
    appToast('CSV exported');
  };
  ...
  const exportPdf = async () => {
    ...
    const pdf = new jsPDF();
    pdf.setFontSize(18);
    pdf.text(`Wersee tax summary ${reportYear}`, 14, 20);
    pdf.setFontSize(11);
    [
      `Reconciled revenue: EUR ${stats.grossRevenue.toFixed(2)}`,
      `Stripe fees: EUR ${stats.stripeFees.toFixed(2)}`,
      `Wersee fees: EUR ${stats.werseeFees.toFixed(2)}`,
      `Tax recorded: EUR ${(taxData?.taxReserve || 0).toFixed(2)}`,
      `Net: EUR ${(taxData?.netProfit || 0).toFixed(2)}`,
      `Matched transactions: ${stats.totalTransactions}`,
    ].forEach((line, index) => pdf.text(line, 14, 34 + index * 8));
    pdf.save(`wersee-tax-summary-${reportYear}.pdf`);
  };

  const handleExport = () => {
    downloadBlob(`wersee-tax-audit-${reportYear}.json`, 'application/json', JSON.stringify({
    ...
    appToast('Audit package exported');
  };
        ...
        <div className="bg-[#141414] border border-white/5 rounded-2xl md:rounded-[2.5rem] p-6 md:p-8 relative overflow-hidden group">
          ...
          <div className="space-y-1">
            ...
            <p className={`text-xs md:text-sm flex items-center gap-1 ${
              trendPercent === null || trendPercent === 0 ? 'text-gray-500' : trendPercent > 0 ? 'text-emerald-400' : 'text-red-400'
            }`}>
              {trendPercent === null || trendPercent === 0
                ? <span>— No comparable prior-year revenue</span>
                : trendPercent > 0
                  ? <><ArrowUpRight className="w-3 h-3 md:w-4 md:h-4" /> +{trendPercent.toFixed(1)}% from last year</>
                  : <><ArrowDownRight className="w-3 h-3 md:w-4 md:h-4" /> {trendPercent.toFixed(1)}% from last year</>}
            </p>
          </div>
        </div>
        ...
        <div className="bg-[#141414] border border-white/5 rounded-2xl md:rounded-[2.5rem] p-6 md:p-8 flex flex-col">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <h3 className="text-lg md:text-xl font-bold text-white">Reconciliation Log</h3>
                <p className="text-xs md:text-sm text-gray-500">Stripe + Supabase exact matches</p>
              </div>
            </div>
            {analyzing && (
              ...
              {transactions.slice(0, 3).map((t, i) => (
                <div key={i} className="flex items-center justify-between p-2.5 md:p-3 rounded-lg md:rounded-xl bg-white/[0.02] border border-white/5 text-[10px] md:text-xs">
                  <div className="flex items-center gap-2 md:gap-3">
                    ...
                    <span className="text-gray-400 font-mono">{String(t.id).slice(0, 12)}</span>
                    <span className="text-gray-500">{String(t.currency || 'eur').toUpperCase()} {Number(t.amount || 0).toFixed(2)}</span>
                  </div>
                  <span className="text-gray-600 hidden sm:inline">Verified Match</span>
                </div>
              ))}
              {transactions.length === 0 && <p className="px-2 text-xs text-gray-500">No reconciled payments for this period.</p>}
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3 md:gap-4">
            <button onClick={exportCsv} className="flex items-center justify-center gap-2 p-3 md:p-4 rounded-xl md:rounded-2xl bg-white/5 text-white hover:bg-white/10 transition-all text-xs md:text-sm font-bold">
              <FileSpreadsheet className="w-3.5 h-3.5 md:w-4 md:h-4" />
              CSV Export
            </button>
            <button onClick={() => void exportPdf()} className="flex items-center justify-center gap-2 p-3 md:p-4 rounded-xl md:rounded-2xl bg-white/5 text-white hover:bg-white/10 transition-all text-xs md:text-sm font-bold">
