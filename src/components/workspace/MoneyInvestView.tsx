...
export const MoneyInvestView = () => {
  ...
  const fetchInvestData = async () => {
    try {
      ...
      // 1. Fetch Total Profit from Orders
      ...
      const profit = orders?.reduce((sum, order) => sum + Number(order.amount), 0) || 0;
      ...
      if (settings) {
        ...
        setProfitPercentage(settings.profit_allocation);
        setWerseeControl(settings.wersee_control);
        setSelectedStrategy(settings.strategy);
        setAllocation(settings.allocation_percentage);
        setReinvest(settings.reinvest_profits);
      ...
      if (perfData && perfData.length > 0) {
        ...
        setPerformance(performanceData); // Fallback to mock
      }
      ...
      if (poolsData && poolsData.length > 0) {
        ...
        setPools(activeInvestments); // Fallback to mock
      }
  ...
  const handleActivate = async () => {
    try {
      ...
      const settingsData = {
        user_id: user.id,
        is_active: true,
        profit_allocation: profitPercentage,
