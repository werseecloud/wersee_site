...
export const ProposalsView: React.FC = () => {
  ...
  const fetchProposals = async () => {
    try {
      ...
      const { data: financeBusiness, error: businessError } = await supabase.rpc('ensure_finance_business');
      if (businessError) throw businessError;
      const resolvedBusiness = Array.isArray(financeBusiness) ? financeBusiness[0] : financeBusiness;
      const businessIdToUse = resolvedBusiness?.business_id;
      setOwnerUsername((resolvedBusiness?.username || '').replace(/^@/, ''));
  ...
  const handleGeneratePdf = async (proposalId: string) => {
    try {
      ...
      const basePath = ownerUsername
        ? `/${encodeURIComponent(`@${ownerUsername}`)}/proposal/${proposalId}`
        : `/proposal/${proposalId}`;
      window.open(`${basePath}?download=1`, '_blank', 'noopener,noreferrer');
    } catch (err) {
      console.error('Error generating PDF:', err);
      appToast('Failed to generate PDF. Please try again.');
    } finally {
      window.setTimeout(() => setIsGeneratingPdf(null), 800);
    }
  };
