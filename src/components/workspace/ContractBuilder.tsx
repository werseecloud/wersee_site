...
export const ContractBuilder: React.FC<ContractBuilderProps> = ({ contractId, onClose }) => {
  ...
  const [contract, setContract] = useState<any>({
    ...
    metadata: { auto_quick_link: false, payment_amount: '', payment_currency: 'eur' }
  });
  ...
  const handleSend = async (retries = 3) => {
    ...
    setSaving(true);
    try {
      if (!contract.client_name?.trim() || !contract.client_email?.trim()) {
        throw new Error('Add the client name and email before sending.');
      }
      if (contract.metadata?.auto_quick_link && !(Number(contract.metadata?.payment_amount) > 0)) {
        throw new Error('Add a payment amount for Auto Quick Link.');
      }
      const payload = {
        title: contract.title,
        type: contract.type,
      ...
      appToast(error instanceof Error ? error.message : 'Failed to send contract.');
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="flex-1 flex items-center justify-center p-8 overflow-y-auto z-10">
            <div className="w-full max-w-md bg-[#141414] rounded-3xl shadow-2xl border border-white/5 p-10">
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/[0.06] p-4 space-y-4">
                  <label className="flex items-start justify-between gap-4 cursor-pointer">
                    <div>
                      <p className="text-sm font-bold text-white">When signed → Auto Quick Link</p>
                      <p className="mt-1 text-xs leading-relaxed text-gray-400">
                        After the client signs, Wersee emails a secure payment page from pay@wersee.com with the contract attached.
                      </p>
                    </div>
                    <input
                      type="checkbox"
                      checked={Boolean(contract.metadata?.auto_quick_link)}
                      onChange={(event) => setContract({
                        ...contract,
                        metadata: { ...contract.metadata, auto_quick_link: event.target.checked },
                      })}
                      className="mt-1 h-5 w-5 accent-emerald-500"
                    />
                  </label>
                  {contract.metadata?.auto_quick_link && (
                    <div className="grid grid-cols-[1fr_110px] gap-3">
                      <input
                        type="number"
                        min="0.50"
                        step="0.01"
                        value={contract.metadata?.payment_amount || ''}
                        onChange={(event) => setContract({
                          ...contract,
                          metadata: { ...contract.metadata, payment_amount: event.target.value },
                        })}
                        placeholder="Amount"
                        className="w-full px-4 py-3 bg-black/20 border border-white/10 rounded-xl text-white focus:outline-none focus:border-emerald-400"
                      />
                      <Select
                        value={contract.metadata?.payment_currency || 'eur'}
                        onChange={(value) => setContract({
                          ...contract,
                          metadata: { ...contract.metadata, payment_currency: value },
                        })}
                        options={[
                          { value: 'eur', label: 'EUR' },
                          { value: 'usd', label: 'USD' },
