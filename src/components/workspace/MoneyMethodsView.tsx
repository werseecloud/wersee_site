...
export const MoneyMethodsView = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      if (business) {
        ...
        const { data: profile } = await supabase.from('profiles').select('company_name,full_name,username').eq('id', user.id).maybeSingle();
        const { data: created, error: createError } = await supabase
          .from('businesses')
          .insert({
            user_id: user.id,
            name: profile?.company_name || profile?.full_name || profile?.username || 'My business',
            supported_payment_methods: ['card', 'wallet'],
          })
          .select('id')
          .single();
        if (createError) {
          appToast('Create a business profile before saving payment methods.');
        } else {
  ...
  const handleSave = async () => {
    if (!businessId) {
      appToast('No business profile is available for these payment methods.');
      return;
    }
    setSaving(true);
    try {
