...
export const WikiView: React.FC<WikiViewProps> = ({ teamId }) => {
  ...
  const handleCreateArticle = async (categoryId?: string) => {
    try {
      ...
      appToast(error instanceof Error ? error.message : 'The article could not be created');
    }
  };
  ...
  const handleSaveArticle = async () => {
    ...
    try {
      ...
      if (error) throw error;
      const savedArticle = { ...selectedArticle, updated_at: new Date().toISOString() };
      setSelectedArticle(savedArticle);
      setArticles(articles.map(a => a.id === selectedArticle.id ? savedArticle : a));
      setSaveState('saved');
      ...
      setSaveState('error');
      appToast(error instanceof Error ? error.message : 'The article could not be saved');
    }
  };
          ...
          <div className="flex items-center gap-2">
            ...
            {isEditing ? (
              <SaveStateButton
                ...
                disabled={!selectedArticle.title.trim()}
                state={saveState}
