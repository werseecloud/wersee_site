...
import {
  ...
  resolveWerseeFile,
  uploadWerseeFile,
  type WerseeStoredObject,
} from '../../services/werseeStorage';
...
interface StorageItem {
  ...
  url?: string;
  isStarred?: boolean;
  storedObject?: WerseeStoredObject;
}
...
export const StorageView = () => {
  ...
  const fetchStorage = async () => {
    ...
    try {
      ...
      const result = await listWerseeFiles('business_storage', prefix);
      let formattedItems: StorageItem[] = [
        ...result.folders.map((name) => ({
          id: `folder:${[...currentPath, name].join('/')}`,
          name,
          isFolder: true,
          updatedAt: new Date(0).toISOString(),
          path: [...currentPath, name].join('/'),
          isStarred: starredIds.has(`folder:${[...currentPath, name].join('/')}`),
        })),
        ...result.objects.map((object) => ({
          id: object.objectId,
          name: object.logicalPath.split('/').pop() || object.logicalPath,
          isFolder: false,
          size: object.sizeBytes,
          type: object.mimeType,
          updatedAt: object.updatedAt || object.createdAt || new Date(0).toISOString(),
          path: object.logicalPath,
          url: object.url || undefined,
          isStarred: starredIds.has(object.objectId),
          storedObject: object,
        })),
      ];
  ...
  const uploadFiles = async (files: File[]) => {
    ...
    try {
      for (const file of files) {
        ...
        const logicalPath = [...currentPath, relativePath].join('/');
        try {
          await uploadWerseeFile({
            bucketId: 'business_storage',
            logicalPath,
            file,
            concurrency: 2,
            onProgress: ({ acknowledgedBytes }) => {
              setUploadProgress(Math.round(((totalUploaded + acknowledgedBytes) / totalSize) * 100));
            },
          });
          ...
          appToast(`Upload failed for ${file.name}. You can retry without losing acknowledged progress.`);
          completedFiles++;
        }
      }
    } catch (err) {
  ...
  const handleDownload = async (item: StorageItem) => {
    ...
    if (item.isFolder) {
      ...
      const object = item.storedObject || await resolveWerseeFile('business_storage', item.path);
      saveAs(await downloadWerseeFile(object), item.name);
    }
  };
  ...
  const deleteFolderContents = async (folderPath: string) => {
    const result = await listWerseeFiles('business_storage', folderPath);
    for (const child of result.folders) await deleteFolderContents(`${folderPath}/${child}`);
    for (const object of result.objects) await deleteWerseeFile(object);
  };
  ...
  const handleRename = async () => {
    ...
    setLoading(true);
    try {
      if (selectedItem.isFolder) throw new Error('FOLDER_RENAME_REQUIRES_JOB');
      const object = selectedItem.storedObject || await resolveWerseeFile('business_storage', selectedItem.path);
      const newPath = [...currentPath, newName].join('/');
      await moveWerseeFile(object, newPath);
  ...
  const handleCreateFolder = async () => {
    ...
    setLoading(true);
    try {
      appToast('The folder will be created when its first file is uploaded.');
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      ...
      for (const item of itemsToDelete) {
        if (item.isFolder) await deleteFolderContents(item.path);
        else await deleteWerseeFile(item.storedObject || await resolveWerseeFile('business_storage', item.path));
      }
  ...
  const openViewer = async (item: StorageItem) => {
    ...
    const object = item.storedObject || await resolveWerseeFile('business_storage', item.path);
    setSelectedItem({ ...item, storedObject: object, url: await downloadWerseeFile(object) });
    setViewerOpen(true);
  };
