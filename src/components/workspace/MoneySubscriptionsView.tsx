...
export const MoneySubscriptionsView = () => {
  ...
  const subscriptionUrl = (subscription: any) => {
    const resolvedUsername = (subscription?.username || username || '').replace(/^@/, '');
    if (!resolvedUsername) return `${window.location.origin}/subscriptions/${encodeURIComponent(subscription.slug)}`;
    return `${window.location.origin}/${encodeURIComponent(`@${resolvedUsername}`)}/subscriptions/${encodeURIComponent(subscription.slug)}`;
  };
    ...
    const init = async () => {
      ...
      if (user) {
        ...
        const [{ data: profile }, { data: payoutProfile }] = await Promise.all([
          supabase.from('profiles').select('username,stripe_account_id').eq('id', user.id).maybeSingle(),
          supabase.from('seller_payout_profiles').select('stripe_account_id').eq('user_id', user.id).maybeSingle(),
        ]);
        setUsername((profile?.username || user.user_metadata?.username || user.email?.split('@')[0] || '').replace(/^@/, ''));
        setHasStripeAccount(Boolean(profile?.stripe_account_id || payoutProfile?.stripe_account_id));
        await Promise.all([fetchSubscriptions(user.id), fetchSubscribers(user.id)]);
      }
      setLoading(false);
    };
    init();
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      appToast(err instanceof Error ? err.message : 'AI generation failed');
    } finally {
  ...
  const handleCreateSubscription = async () => {
    ...
    try {
      if (!formData.name.trim() || !(Number(formData.price) > 0)) {
        throw new Error('Add a plan name and an amount above zero.');
      }

      await invokeFinanceWorkflow('subscription-create', formData);

      setShowWizard(false);
      if (userId) fetchSubscriptions(userId);
      appToast('Subscription created');
      // Reset form
      setFormData({
        name: '',
        description: '',
            ...
            {subscriptions.map((sub) => (
              <div key={sub.id} className="bg-[#141414] border border-white/5 rounded-2xl p-5 space-y-4 group hover:border-white/10 transition-all">
                ...
                <div className="pt-4 border-t border-white/5 flex items-center justify-between gap-2">
                  <button 
                    onClick={() => copyToClipboard(subscriptionUrl(sub), sub.id)}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center text-[10px] font-bold text-white">
                            {(sub.buyer_name || sub.buyer_email || sub.user_id || '?').slice(0, 2).toUpperCase()}
                          </div>
                          <div>
                            <p className="text-gray-200 font-medium">{sub.buyer_name || sub.buyer_email || 'Subscriber'}</p>
                            {sub.buyer_name && sub.buyer_email && <p className="text-[10px] text-gray-500">{sub.buyer_email}</p>}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
