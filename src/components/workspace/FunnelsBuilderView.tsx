...
import { Stage, Layer, Rect, Circle, Text, Group, Arrow, Shape } from 'react-konva';
...
export const FunnelsBuilderView = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error } = await supabase.rpc('save_funnel_graph', {
        p_funnel_id: currentFunnel.id,
        p_nodes: nodesToInsert,
        p_connections: connectionsToInsert
      });
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!saved) return;
      const token = currentFunnel.share_token || crypto.randomUUID();
      const updated = await DatabaseService.update<Funnel>('funnels', currentFunnel.id, {
        share_token: token,
        is_public: true,
        exported_at: new Date().toISOString()
      });
      if (!updated) throw new Error('The public link could not be created.');
      const url = `${window.location.origin}/funnel/${encodeURIComponent(token)}`;
      setCurrentFunnel(updated);
      setFunnels(current => current.map(funnel => funnel.id === updated.id ? updated : funnel));
      setShareUrl(url);
      setShowShare(true);
      ...
      toast.error(error?.message || 'Failed to export the funnel.');
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleWheel = (e: any) => {
    ...
    const newScale = Math.min(2.5, Math.max(0.35, e.evt.deltaY < 0 ? oldScale * scaleBy : oldScale / scaleBy));
    setScale(newScale);
    setPosition({
  ...
  const uniqueConnections = useMemo(
    () => Array.from(new Map(connections.map(connection => [connection.id, connection])).values()),
    [connections]
  );
      ...
      <div className="h-16 md:h-16 border-b border-white/5 bg-[#111] px-4 md:px-6 flex items-center justify-between shrink-0 z-30">
        ...
        <div className="flex items-center gap-1 md:gap-3">
          ...
          <button
            ...
            className="p-2 md:px-4 md:py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-xl text-sm font-bold transition-all flex items-center gap-2 shadow-lg shadow-indigo-500/20 disabled:opacity-50"
          >
            {exporting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Share2 className="w-4 h-4" />} <span className="hidden md:inline">Export</span>
          </button>
        </div>
      </div>
          ...
          <Layer>
            <Shape
              ...
              sceneFunc={(context, shape) => {
                const spacing = 40;
                const minX = Math.floor((-position.x / scale - spacing) / spacing) * spacing;
                const minY = Math.floor((-position.y / scale - spacing) / spacing) * spacing;
                const maxX = minX + stageSize.width / scale + spacing * 2;
                const maxY = minY + stageSize.height / scale + spacing * 2;
                context.beginPath();
            ...
            {uniqueConnections.map(conn => {
              const from = uniqueNodes.find(n => n.id === conn.from);
              const to = uniqueNodes.find(n => n.id === conn.to);
              if (!from || !to) return null;
              
              const startX = from.x + 150;
              const startY = from.y + 75;
              const endX = to.x;
              const endY = to.y + 75;
              ...
              return (
                <Group key={conn.id}>
                  <Arrow
                    points={[startX, startY, startX + bend, startY, endX - bend, endY, endX, endY]}
                    stroke="#4F46E5"
                    strokeWidth={2.5}
                    fill="#4F46E5"
                    bezier
                  ...
                  />
                  {showAnalytics && (
                    <Group x={midX - 24} y={midY - 11}>
                      <Rect width={48} height={22} fill="#111" cornerRadius={6} stroke="#4F46E5" strokeWidth={1} />
                      <Text text={`${from.stats?.conversion ?? 0}%`} fontSize={10} fill="#A5B4FC" width={48} align="center" padding={6} />
                    </Group>
                  )}
                </Group>
              );
          ...
          {showShare && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                ...
                <h2 className="text-2xl font-black text-white">Your funnel is ready to share</h2>
                <p className="mt-2 text-sm leading-6 text-gray-400">Anyone with this link can view the published funnel without editing it.</p>
                <div className="mt-6 flex items-center gap-2 rounded-2xl border border-white/10 bg-black/30 p-3">
                  <code className="min-w-0 flex-1 truncate text-xs text-indigo-300">{shareUrl}</code>
                  <button
                    type="button"
                    onClick={() => navigator.clipboard.writeText(shareUrl).then(() => toast.success('Link copied'))}
                    className="rounded-xl bg-white/10 p-3 text-white hover:bg-white/15"
                  >
                    <Copy className="h-4 w-4" />
                  </button>
                </div>
                <div className="mt-6 flex gap-3">
                  <button type="button" onClick={() => setShowShare(false)} className="flex-1 rounded-xl bg-white/5 px-5 py-3 font-bold text-white hover:bg-white/10">Done</button>
                  <button type="button" onClick={() => window.open(shareUrl, '_blank', 'noopener,noreferrer')} className="flex-1 rounded-xl bg-indigo-500 px-5 py-3 font-bold text-white hover:bg-indigo-600">Open link</button>
                </div>
              </motion.div>
            </div>
          )}
