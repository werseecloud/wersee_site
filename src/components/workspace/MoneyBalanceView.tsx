  ...
  const fetchBalanceAndTransactions = async () => {
    ...
    try {
      ...
      if (profile?.stripe_account_id) {
        const resData = await invokeApiRunner('stripe-balance-get', { accountId: profile.stripe_account_id });
        if (resData.error) {
          throw new Error(resData.error || 'Failed to fetch balance');
        }
        setBalance(resData);
      } else {
    ...
    <div className="space-y-4 md:space-y-8 p-4 md:p-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={() => window.location.href = '/workspace/overview?view=money-payouts'}
