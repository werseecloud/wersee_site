...
import { CampaignPromotionCheckout } from './CampaignPromotionCheckout';
...
export const AdsManager = () => {
  ...
  const [checkoutCampaign, setCheckoutCampaign] = useState<any | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      if (analyticsData) {
        ...
        const totalsFor = (from: Date, to: Date) => analyticsData
          .filter(item => {
        ...
        const current = totalsFor(startCurrent, new Date(now.getTime() + 86400000));
        const previous = totalsFor(startPrevious, startCurrent);
        const growth = (value: number, prior: number) => prior > 0 ? Math.round(((value - prior) / prior) * 100) : value > 0 ? 100 : 0;
        
        setStats({
          impressions: totalImpressions,
          clicks: totalClicks,
          conversions: totalConversions,
          impressionsGrowth: growth(current.impressions, previous.impressions),
          clicksGrowth: growth(current.clicks, previous.clicks),
          conversionsGrowth: growth(current.conversions, previous.conversions)
        });
      }
    ...
    try {
      ...
      const { data: campaign, error } = await supabase
        ...
        .insert({
          user_id: user.id,
          ...formData,
          status: formData.promote_on_wersee ? 'paused' : 'active',
          promotion_status: formData.promote_on_wersee ? 'pending_payment' : 'not_requested',
        })
        .select('*')
        .single();
      ...
      setShowWizard(false);
      await fetchData();
      if (formData.promote_on_wersee && campaign) setCheckoutCampaign(campaign);
      setFormData({
        title: '',
        budget_daily: 5,
        type: 'search',
        promote_on_wersee: false,
        targeting: { audience: 'all', location: 'global' }
      });
    } catch (error) {
      console.error('Error creating campaign:', error);
      appToast('Failed to create campaign', 'error');
    } finally {
      setCreating(false);
    }
  };
  ...
  const toggleStatus = async (id: string, currentStatus: string) => {
    ...
    if (campaign?.promote_on_wersee && campaign?.promotion_status !== 'active') {
      setCheckoutCampaign(campaign);
      appToast('Complete the verified payment before activating this promotion.', 'warning');
      return;
    }
    ...
    <div className="max-w-7xl mx-auto pb-20 p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-[#141414] p-6 rounded-2xl border border-white/5 shadow-sm">
          ...
          <div className={`text-xs font-medium mt-1 flex items-center gap-1 ${stats.impressionsGrowth >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            <TrendingUp className="w-3 h-3" /> {stats.impressionsGrowth > 0 ? '+' : ''}{stats.impressionsGrowth}% this week
          </div>
        </div>
        <div className="bg-[#141414] p-6 rounded-2xl border border-white/5 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
          ...
          <div className={`text-xs font-medium mt-1 flex items-center gap-1 ${stats.clicksGrowth >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            <TrendingUp className="w-3 h-3" /> {stats.clicksGrowth > 0 ? '+' : ''}{stats.clicksGrowth}% this week
          </div>
        </div>
        <div className="bg-[#141414] p-6 rounded-2xl border border-white/5 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
          ...
          <div className={`text-xs font-medium mt-1 flex items-center gap-1 ${stats.conversionsGrowth >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            <TrendingUp className="w-3 h-3" /> {stats.conversionsGrowth > 0 ? '+' : ''}{stats.conversionsGrowth}% this week
          </div>
        </div>
      </div>
            ...
            {campaigns.map((camp) => (
              <div key={camp.id} className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-white/5 transition-colors">
                <div className="col-span-4 pl-2 font-medium text-white">
                  {camp.title}
                  {camp.promote_on_wersee && (
                    <button onClick={() => camp.promotion_status === 'pending_payment' && setCheckoutCampaign(camp)} className="ml-2 rounded-full border border-indigo-400/20 bg-indigo-400/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-indigo-300">
                      {camp.promotion_status === 'active' ? 'Promoted' : camp.promotion_status === 'pending_payment' ? 'Complete payment' : camp.promotion_status || 'Wersee'}
                    </button>
                  )}
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="p-8 space-y-6">
              ...
              <label className={`block cursor-pointer rounded-2xl border p-5 transition-all ${formData.promote_on_wersee ? 'border-fuchsia-400/40 bg-gradient-to-br from-indigo-500/15 to-fuchsia-500/10' : 'border-white/10 bg-black/20 hover:border-white/20'}`}>
                <div className="flex items-start gap-4">
                  <input type="checkbox" checked={formData.promote_on_wersee} onChange={event => setFormData({ ...formData, promote_on_wersee: event.target.checked })} className="mt-1 h-4 w-4 accent-indigo-500" />
                  <div className="flex-1">
                    <div className="flex items-center gap-2 font-black text-white"><Sparkles className="h-4 w-4 text-fuchsia-300" /> Promote on Wersee</div>
                    <p className="mt-1 text-sm leading-6 text-white/45">Get seven days of placement in relevant Wersee search and community discovery, with impressions, clicks and conversions in this dashboard.</p>
                    <div className="mt-3 flex items-center justify-between rounded-xl bg-black/25 px-4 py-3">
                      <span className="text-xs font-bold uppercase tracking-wider text-white/35">7-day launch budget</span>
                      <span className="font-black text-white">€{Math.max(10, Number(formData.budget_daily || 0) * 7).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </label>
            ...
            <div className="p-6 border-t border-white/10 flex justify-end gap-3 bg-[#141414]">
              ...
              >
                {creating ? <Loader2 className="w-4 h-4 animate-spin" /> : formData.promote_on_wersee ? 'Continue to payment' : 'Launch Campaign'}
              </button>
            </div>
          </motion.div>
        </div>
