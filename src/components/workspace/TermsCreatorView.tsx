...
const resolveTermsBusinessId = async (userId: string) => {
  ...
  if (ownedBusiness?.id) return ownedBusiness.id as string;
  ...
  return teamMember?.business_id as string | undefined;
};

export const TermsCreatorView: React.FC<{ businessId?: string }> = ({ businessId }) => {
  ...
  const fetchTerms = async () => {
    try {
      ...
      const businessIdToUse = businessId || await resolveTermsBusinessId(user.id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsDefault(false);
      await fetchTerms();
      appToast(editingTerm ? 'Terms updated.' : 'Terms created.', 'success');
    } catch (err) {
