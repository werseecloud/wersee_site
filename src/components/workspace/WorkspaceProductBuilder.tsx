...
import { 
  ...
  ChevronRight, Monitor, Smartphone, Tablet, ShieldCheck
} from 'lucide-react';
...
import {
  ...
  isDsaVerificationComplete,
  requiresDsaSellerVerification,
...
export const WorkspaceProductBuilder: React.FC<WorkspaceProductBuilderProps> = ({ 
  ...
  const [dsaVerification, setDsaVerification] = useState<any>(null);
  const [dsaForm, setDsaForm] = useState({
    country_code: getEffectiveDsaCountryCode('NL') || 'NL',
    trader_status: 'consumer' as 'consumer' | 'business',
    legal_name: '',
  ...
  const [safetyForm, setSafetyForm] = useState({
    ...
    safety_warnings: '',
    intended_use: '',
    recall_contact_email: user?.email || '',
  });
  ...
  React.useEffect(() => {
    ...
    const loadDsaVerification = async () => {
      const existing = await getCurrentSellerDsaVerification(user.id);
      if (!existing) return;
      setDsaVerification(existing);
      setDsaForm((prev) => ({
        ...prev,
        country_code: existing.country_code || prev.country_code,
        trader_status: existing.trader_status || prev.trader_status,
        legal_name: existing.legal_name || prev.legal_name,
        contact_email: existing.contact_email || prev.contact_email,
        registered_address: existing.registered_address || prev.registered_address,
        registration_number: existing.registration_number || prev.registration_number,
        vat_or_tax_id: existing.vat_or_tax_id || prev.vat_or_tax_id,
      }));
    };

    loadDsaVerification().catch((error) => {
      console.warn('Could not load DSA seller verification:', error);
    });
  }, [user?.id]);
  ...
  const fetchProduct = async (retries = 3) => {
    try {
      ...
      if (data) {
        ...
        if (['physical', 'pos'].includes(data.type)) {
          const safety = await DatabaseService.get('product_safety_profiles', { eq: { listing_id: initialProductId }, single: true }).catch(() => null);
          if (safety) setSafetyForm((current) => ({
            ...current,
            manufacturer_name: safety.manufacturer_name || '',
            manufacturer_address: safety.manufacturer_address || '',
            manufacturer_email: safety.manufacturer_email || '',
            eu_responsible_person_name: safety.eu_responsible_person_name || '',
            eu_responsible_person_contact: safety.eu_responsible_person_contact || '',
            product_identifier: safety.product_identifier || '',
            country_of_origin: safety.country_of_origin || '',
            safety_warnings: safety.safety_warnings || '',
            intended_use: safety.intended_use || '',
            recall_contact_email: safety.recall_contact_email || '',
          }));
        }
      }
    } catch (error: any) {
  ...
  const handleSave = async (statusOverride?: 'draft' | 'published') => {
    ...
    try {
      ...
      const isSafetyProduct = ['physical', 'pos'].includes(formData.type);
      const safetyComplete = Boolean(
        safetyForm.manufacturer_name && safetyForm.manufacturer_address && safetyForm.manufacturer_email &&
        safetyForm.product_identifier && safetyForm.safety_warnings && safetyForm.recall_contact_email
      );
      if (finalStatus === 'published' && isSafetyProduct && !safetyComplete) {
        setStep(4);
        appToast('Complete the product-safety and recall details before requesting publication.');
        return;
      }
      const dsaRequired = requiresDsaSellerVerification({
        countryCode: dsaForm.country_code,
        traderStatus: dsaForm.trader_status,
      });
      const hasDsaVerification = isDsaVerificationComplete(dsaVerification) || (
        dsaRequired &&
        Boolean(dsaForm.legal_name && dsaForm.contact_email && dsaForm.registered_address)
      );

      if (finalStatus === 'published' && dsaRequired && !hasDsaVerification) {
        setStep(5);
        appToast('Complete EU seller verification before publishing this marketplace item.');
        return;
      }

      const submittedDsaVerification = dsaRequired || dsaForm.trader_status === 'consumer'
        ? await upsertDsaSellerVerification({
          seller_id: user.id,
          trader_status: dsaForm.trader_status,
          legal_name: dsaForm.legal_name || user.email || 'Private seller',
          contact_email: dsaForm.contact_email || user.email || '',
          country_code: dsaForm.country_code,
          registered_address: dsaForm.registered_address || null,
          registration_number: dsaForm.registration_number || null,
          vat_or_tax_id: dsaForm.vat_or_tax_id || null,
          status: dsaRequired ? 'pending' : 'not_required',
        })
        : dsaVerification;

      setDsaVerification(submittedDsaVerification);
      ...
      const listingData = {
        ...
        dsa_seller_verification_status: submittedDsaVerification?.status || 'unknown',
        metadata: {
          ...
          dsa_seller_country: dsaForm.country_code,
          dsa_trader_status: dsaForm.trader_status,
        }
      };
      
      let listingId = initialProductId || '';
      if (initialProductId) {
        if (isSafetyProduct) await DatabaseService.upsert('product_safety_profiles', {
          user_id: user.id, listing_id: initialProductId, ...safetyForm,
          product_identifier: safetyForm.product_identifier || formData.sku || formData.barcode,
          status: safetyComplete ? 'complete' : 'incomplete', policy_version: 'trust-core-2026-07-22', created_by: user.id,
        }, 'listing_id');
        await DatabaseService.update('listings', initialProductId, listingData);
      } else if (isSafetyProduct) {
        const draftListing = await DatabaseService.insert<any>('listings', { ...listingData, status: 'draft' });
        listingId = draftListing.id;
        await DatabaseService.upsert('product_safety_profiles', {
          user_id: user.id, listing_id: listingId, ...safetyForm,
          product_identifier: safetyForm.product_identifier || formData.sku || formData.barcode,
          status: safetyComplete ? 'complete' : 'incomplete', policy_version: 'trust-core-2026-07-22', created_by: user.id,
        }, 'listing_id');
        if (finalStatus === 'published') await DatabaseService.update('listings', listingId, { status: 'published' });
      } else {
      ...
      appToast(error?.message?.includes('Trust Center') ? 'Publication is paused by Trust Center review. Your product remains a draft.' : error?.message || 'The product could not be saved.');
    } finally {
      setIsSaving(false);
    }
  };
            ...
            {(formData.type === 'physical' || formData.type === 'pos') && (
              <div className="space-y-5">
                ...
                <div className="rounded-2xl border border-amber-400/20 bg-amber-400/[0.05] p-4">
                  <p className="text-sm font-bold text-amber-100">Product safety and recall details</p>
                  <p className="mt-1 text-xs leading-5 text-amber-100/55">Required fields are checked before publication. Physical-product sales remain disabled until the applicable Trust Operations review and feature gate are active.</p>
                </div>
                <CustomInput label="Manufacturer name" value={safetyForm.manufacturer_name} onChange={(e: any) => setSafetyForm({ ...safetyForm, manufacturer_name: e.target.value })} />
                <CustomTextarea label="Manufacturer postal address" value={safetyForm.manufacturer_address} onChange={(e: any) => setSafetyForm({ ...safetyForm, manufacturer_address: e.target.value })} />
                <div className="grid grid-cols-2 gap-4">
                  <CustomInput label="Manufacturer email" type="email" value={safetyForm.manufacturer_email} onChange={(e: any) => setSafetyForm({ ...safetyForm, manufacturer_email: e.target.value })} />
                  <CustomInput label="Product identifier" value={safetyForm.product_identifier} onChange={(e: any) => setSafetyForm({ ...safetyForm, product_identifier: e.target.value })} placeholder="Model, SKU or EAN" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <CustomInput label="Country of origin" value={safetyForm.country_of_origin} onChange={(e: any) => setSafetyForm({ ...safetyForm, country_of_origin: e.target.value })} />
                  <CustomInput label="Recall contact email" type="email" value={safetyForm.recall_contact_email} onChange={(e: any) => setSafetyForm({ ...safetyForm, recall_contact_email: e.target.value })} />
                </div>
                <CustomTextarea label="Safety warnings" value={safetyForm.safety_warnings} onChange={(e: any) => setSafetyForm({ ...safetyForm, safety_warnings: e.target.value })} />
                <CustomTextarea label="Intended use" value={safetyForm.intended_use} onChange={(e: any) => setSafetyForm({ ...safetyForm, intended_use: e.target.value })} />
                <div className="grid grid-cols-2 gap-4">
                  <CustomInput label="EU responsible person" value={safetyForm.eu_responsible_person_name} onChange={(e: any) => setSafetyForm({ ...safetyForm, eu_responsible_person_name: e.target.value })} />
                  <CustomInput label="EU responsible contact" value={safetyForm.eu_responsible_person_contact} onChange={(e: any) => setSafetyForm({ ...safetyForm, eu_responsible_person_contact: e.target.value })} />
                </div>
              </div>
            )}
        ...
        return (
          <div className="space-y-8 py-4">
            ...
            <div className="flex items-center gap-4 p-4 bg-indigo-500/10 rounded-2xl border border-indigo-500/20">
              ...
              <div>
                <div className="font-bold text-indigo-400">Ready for validation?</div>
                <div className="text-xs text-indigo-400/60">Server-side rules decide whether publication can proceed or needs review.</div>
              </div>
            </div>
            <div className="p-6 bg-white/5 rounded-2xl border border-white/10 space-y-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold uppercase tracking-widest text-gray-300">EU Seller Verification</h3>
                  <p className="mt-1 text-xs text-gray-500">Required when an EU seller lists marketplace goods or services as a business.</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <CustomSelect
                  label="Country"
                  value={dsaForm.country_code}
                  onChange={(e: any) => setDsaForm({ ...dsaForm, country_code: e.target.value })}
                >
                ...
                <CustomSelect
                  label="Seller Type"
                  value={dsaForm.trader_status}
                  onChange={(e: any) => setDsaForm({ ...dsaForm, trader_status: e.target.value })}
                >
                  <option value="consumer">Private seller</option>
                  <option value="business">Business seller</option>
                </CustomSelect>
              </div>
              {requiresDsaSellerVerification({ countryCode: dsaForm.country_code, traderStatus: dsaForm.trader_status }) && (
                <div className="grid gap-4">
                  <CustomInput
                    label="Legal Name"
                    value={dsaForm.legal_name}
                    onChange={(e: any) => setDsaForm({ ...dsaForm, legal_name: e.target.value })}
                  />
                  <CustomInput
                    label="Public Contact Email"
                    type="email"
                    value={dsaForm.contact_email}
                    onChange={(e: any) => setDsaForm({ ...dsaForm, contact_email: e.target.value })}
                  />
                  <CustomInput
                    label="Registered Address"
                    value={dsaForm.registered_address}
                    onChange={(e: any) => setDsaForm({ ...dsaForm, registered_address: e.target.value })}
                  />
                  <div className="grid grid-cols-2 gap-4">
                    <CustomInput
                      label="Registration No."
                      value={dsaForm.registration_number}
                      onChange={(e: any) => setDsaForm({ ...dsaForm, registration_number: e.target.value })}
                    />
                    <CustomInput
                      label="VAT / Tax ID"
                      value={dsaForm.vat_or_tax_id}
                      onChange={(e: any) => setDsaForm({ ...dsaForm, vat_or_tax_id: e.target.value })}
                    />
                  </div>
                </div>
              )}
