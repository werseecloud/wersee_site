    ...
    const load = async () => {
      ...
      try {
        const [
          ...
          supabase
            .from('dsa_notices')
            .select('id,notice_type,status,reason,listing_id,reported_user_id,created_at')
            .order('created_at', { ascending: false })
            .limit(50),
        ]);
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {[
            ...
            ['DSA notices', notices.filter((n) => !['closed', 'rejected'].includes(n.status)).length, ShieldAlert],
        ...
        <div className="rounded-3xl border border-white/10 bg-[#111] p-5">
          <h2 className="mb-4 text-xl font-black">DSA notice-and-action queue</h2>
          <div className="grid gap-3 md:grid-cols-2">
            {notices.length === 0 ? (
              <p className="text-sm text-gray-500">No DSA notices are visible to this role.</p>
            ) : notices.map((notice) => (
              <div key={notice.id} className="rounded-2xl border border-white/10 bg-black/30 p-4">
                <div className="mb-2 flex items-center justify-between gap-3">
                  <p className="font-bold text-white">{notice.notice_type.replace(/_/g, ' ')}</p>
                  <span className="rounded-full bg-blue-500/10 px-2 py-1 text-xs font-bold text-blue-300">{notice.status}</span>
                </div>
                <p className="text-sm text-gray-400">{notice.reason}</p>
                <p className="mt-3 font-mono text-xs text-gray-600">{notice.listing_id || notice.reported_user_id}</p>
              </div>
            ))}
          </div>
        </div>
