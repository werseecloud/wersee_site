const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/authSessionCleanup-aiJ-s9Nh.js","assets/vendor-supabase-DsaUgBUZ.js"])))=>i.map(i=>d[i]);
import{_ as X}from"./vendor-runtime-ckwbz45p.js";import{m as Z,i as W,r as n,j as e,L as C}from"./vendor-react-o_MY0PFO.js";import{u as ee,S as se}from"./app-DIivULxP.js";import{s as d,l as te}from"./authSessionCleanup-aiJ-s9Nh.js";import{aj as ae,b as re,a as oe,m as U,u as ie,$ as q,aw as ne}from"./vendor-ui-DUEZXwNu.js";import{g as P}from"./geminiClient-C_nIw0rz.js";import{c as le}from"./chatAiService-BRSzOi67.js";import"./vendor-supabase-DsaUgBUZ.js";import"./vendor-utils-NQl6BVH0.js";let ce=P();const de=()=>ce;class me{static async getLocalizedResponse(w){const{locale:p,message:o,job_context:j,chat_history:t}=w,f=`
      ...
      ${t?`CHAT HISTORY:
${t}`:""}
    `;try{const c=de();return c?(await c.models.generateContent({model:"gemini-3-flash-preview",contents:f})).text||"I'm sorry, I encountered an error. Could you repeat that?":o}catch(c){return console.error("Localization AI Error:",c),"I'm sorry, I'm having trouble processing your request right now."}}}let ue=P();const pe=()=>ue,Ae=()=>{var E;const{id:u}=Z(),w=W(),{user:p}=ee(),[o,j]=n.useState(null),[t,f]=n.useState(null),[c,g]=n.useState([]),[h,A]=n.useState(""),[M,D]=n.useState(!0),[b,_]=n.useState(!1),S=n.useRef(null);n.useEffect(()=>{u&&p?J():p||w("/auth")},[u,p]),n.useEffect(()=>{var s;(s=S.current)==null||s.scrollIntoView({behavior:"smooth"})},[c]);const J=async()=>{try{const{data:s,error:l}=await d.from("listings").select("*").eq("id",u).single();if(l)throw l;j(s);const r=await te("jobs/apply/start",{jobId:u});r.success&&r.data&&(f(r.data),z(r.data.id))}catch(s){console.error("Error starting application:",s)}finally{D(!1)}},z=async s=>{try{const{supabase:l}=await X(async()=>{const{supabase:x}=await import("./authSessionCleanup-aiJ-s9Nh.js").then(y=>y.x);return{supabase:x}},__vite__mapDeps([0,1])),{data:r}=await l.from("job_application_messages").select("*").eq("application_id",s).order("created_at",{ascending:!0});r&&g(r)}catch(l){console.error("Error fetching messages:",l)}},Y=async s=>{var r,x,y;if(s.preventDefault(),!h.trim()||!t||b)return;const l=h.trim();A(""),_(!0);try{const T=(await le(l)).cleanedContent,{data:B,error:I}=await d.from("job_application_messages").insert({application_id:t.id,message:T,role:"user"}).select().single();if(I)throw I;g(a=>[...a,B]);const{data:i}=await d.from("job_applications").select("*, listings(title, description), job_application_flows(config, job_questions(*))").eq("id",t.id).single(),{data:N}=await d.from("job_application_messages").select("*").eq("application_id",t.id).order("created_at",{ascending:!0}),$=((r=i==null?void 0:i.listings)==null?void 0:r.title)||"a job",F=((x=i==null?void 0:i.listings)==null?void 0:x.description)||"",H=((y=i==null?void 0:i.job_application_flows)==null?void 0:y.job_questions)||[],R=(N==null?void 0:N.map(a=>`${a.role==="ai"?"AI":"Candidate"}: ${a.message}`).join(`
`))||"",k=new URLSearchParams(window.location.search),G=k.get("hl")||k.get("locale")||navigator.language||"en-US",v=await me.getLocalizedResponse({locale:G,message:T,job_context:`Recruiter for "${$}". URL: ${window.location.href}. Description: "${F}". Questions to cover: ${H.map(a=>a.question).join(", ")}`,chat_history:R}),{data:K,error:O}=await d.from("job_application_messages").insert({application_id:t.id,message:v,role:"ai"}).select().single();if(O)throw O;if(g(a=>[...a,K]),v.toLowerCase().includes("application is complete")){f(m=>({...m,status:"completed"}));const a=pe();if(!a){await d.from("job_applications").update({status:"completed"}).eq("id",t.id);return}const V=`
...
//# sourceMappingURL=JobApplication-CTInPDk9.js.map
