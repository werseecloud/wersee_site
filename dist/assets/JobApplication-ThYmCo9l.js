const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/authSessionCleanup-DeKeMjUx.js","assets/vendor-supabase-_AuQRo9U.js"])))=>i.map(i=>d[i]);
import{_ as Z}from"./vendor-runtime-ckwbz45p.js";import{m as W,i as ee,r as n,j as e,L as C}from"./vendor-react-KDWfX7Ci.js";import{u as se,S as te}from"./app-nasSu5Si.js";import{j as U,s as d}from"./authSessionCleanup-DeKeMjUx.js";import{g as M}from"./geminiClient-myalQ_2p.js";import{c as ae}from"./chatAiService-DTebSWJs.js";import{aj as re,b as oe,a as ie,m as q,u as ne,$ as P,aw as le}from"./vendor-ui-7oSf91Z0.js";import"./vendor-supabase-_AuQRo9U.js";import"./vendor-utils-0xY1gjx9.js";let ce=M();const de=()=>ce;class ue{static async getLocalizedResponse(w){const{locale:m,message:o,job_context:j,chat_history:t}=w,f=`
      ...
      ${t?`CHAT HISTORY:
${t}`:""}
    `;try{const l=de();return l?(await l.models.generateContent({model:"gemini-3-flash-preview",contents:f})).text||"I'm sorry, I encountered an error. Could you repeat that?":o}catch(l){return console.error("Localization AI Error:",l),"I'm sorry, I'm having trouble processing your request right now."}}}let me=M();const pe=()=>me,Ae=()=>{var L;const{id:u}=W(),w=ee(),{user:m}=se(),[o,j]=n.useState(null),[t,f]=n.useState(null),[l,g]=n.useState([]),[h,A]=n.useState(""),[D,J]=n.useState(!0),[b,_]=n.useState(!1),S=n.useRef(null);n.useEffect(()=>{u&&m?z():m||w("/auth")},[u,m]),n.useEffect(()=>{var s;(s=S.current)==null||s.scrollIntoView({behavior:"smooth"})},[l]);const z=async()=>{try{const{data:s}=await U("business/get-listing",{id:u});j(s);const r=await U("jobs/apply/start",{jobId:u});r.success&&r.data&&(f(r.data),Y(r.data.id))}catch(s){console.error("Error starting application:",s)}finally{J(!1)}},Y=async s=>{try{const{supabase:r}=await Z(async()=>{const{supabase:x}=await import("./authSessionCleanup-DeKeMjUx.js").then(y=>y.q);return{supabase:x}},__vite__mapDeps([0,1])),{data:p}=await r.from("job_application_messages").select("*").eq("application_id",s).order("created_at",{ascending:!0});p&&g(p)}catch(r){console.error("Error fetching messages:",r)}},B=async s=>{var p,x,y;if(s.preventDefault(),!h.trim()||!t||b)return;const r=h.trim();A(""),_(!0);try{const E=(await ae(r)).cleanedContent,{data:F,error:I}=await d.from("job_application_messages").insert({application_id:t.id,message:E,role:"user"}).select().single();if(I)throw I;g(a=>[...a,F]);const{data:i}=await d.from("job_applications").select("*, listings(title, description), job_application_flows(config, job_questions(*))").eq("id",t.id).single(),{data:N}=await d.from("job_application_messages").select("*").eq("application_id",t.id).order("created_at",{ascending:!0}),$=((p=i==null?void 0:i.listings)==null?void 0:p.title)||"a job",H=((x=i==null?void 0:i.listings)==null?void 0:x.description)||"",G=((y=i==null?void 0:i.job_application_flows)==null?void 0:y.job_questions)||[],R=(N==null?void 0:N.map(a=>`${a.role==="ai"?"AI":"Candidate"}: ${a.message}`).join(`
`))||"",k=new URLSearchParams(window.location.search),K=k.get("hl")||k.get("locale")||navigator.language||"en-US",v=await ue.getLocalizedResponse({locale:K,message:E,job_context:`Recruiter for "${$}". URL: ${window.location.href}. Description: "${H}". Questions to cover: ${G.map(a=>a.question).join(", ")}`,chat_history:R}),{data:V,error:O}=await d.from("job_application_messages").insert({application_id:t.id,message:v,role:"ai"}).select().single();if(O)throw O;if(g(a=>[...a,V]),v.toLowerCase().includes("application is complete")){f(c=>({...c,status:"completed"}));const a=pe();if(!a){await d.from("job_applications").update({status:"completed"}).eq("id",t.id);return}const Q=`
          Analyze the following interview chat history for the position of "${$}".
